<? //проверка существовании сессии
  ...
  if(!empty($_POST['date1'])){
    ...
    AND NOT region = 'Тест'  
    AND datereport 
    BETWEEN '$data1' AND '$data2'  GROUP BY region ");
?>
              ...
              <table id="example" class="tableas table table-hover table-bordered text-center">
                ...
                <tbody style="white-space:nowrap;">
                  <?
                    ...
                    $region = $data['region'];
                    $result13 = R::getCol("SELECT SUM(summstart) FROM repotscom WHERE region ='$region'  AND datereport = '$data1' ");
                    $result14 =  R::getCol("SELECT SUM(endsumm) FROM repotscom WHERE region ='$region' AND datereport = '$data2' ");
                    ?>
                  <tr>
                    <td><?= $i++; ?>.</td>
                    <td>
                      <a type="button" href="a_report.php?id=3&region=<?= $data['region']; ?>&start=<?= $data1; ?>&end=<?= $data2; ?>" class="btn  btn-block bg-olive btn-flat"><?= $data['region']; ?></a>
