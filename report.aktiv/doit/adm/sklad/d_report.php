<? //проверка существовании сессии
  ...
  if(!empty($_GET['start'])){
    ...
    $result = R::getAll("SELECT region,adress,kassa, MIN(summstart), SUM(finhelp),SUM(comis),SUM(proc),SUM(vydacha),SUM(vozvrat),SUM(summsale),SUM(withdrawal),SUM(salesincome),SUM(summstart),SUM(endsumm) 
    FROM repotscom 
    WHERE adress = :adress  
    AND datereport 
    BETWEEN '$data1' AND '$data2'  GROUP BY kassa",[':adress'=>$adress]);
?>
        ...
        <div class="box box-primary">
          ...
          <div class="box-body">
            <form action="a_report.php" method="GET">
              <input type="text" hidden value="4" name="id">
              <input type="text" hidden value="<?= $adress; ?>" name="adress">
              ...
              <table id="example" class="tableas table table-hover table-bordered text-center">
                ...
                <tbody style="white-space:nowrap;">
                  <?$i = 1;
                    ...
                    $kassa = $data['kassa'];
                    $result13 = R::getCol("SELECT SUM(summstart) FROM repotscom WHERE adress = '$adress'  AND kassa ='$kassa'  AND datereport = '$data1' ");
                    $result14 =  R::getAll("SELECT SUM(endsumm),SUM(cash_in_pledge_end) FROM repotscom WHERE adress = '$adress'  AND kassa ='$kassa' AND datereport = '$data2' ");
                    ?>
                  <tr>
                    <td><?= $i++; ?>.</td>
                    <td>
                      <a type="button" href="#" class="btn  btn-block bg-olive btn-flat"><?= $data['kassa']; ?></a>
                    ...
                    <td><?= number_format($result14[0]['SUM(endsumm)'], 0, '.', ' ');
                        $endsumm += $result14[0]['SUM(endsumm)']; ?></td>
                    <td>0</td>
                    ...
                    <td><?= number_format($result14[0]['SUM(cash_in_pledge_end)'], 0, '.', ' ');
                        $cash_in_pledge_end += $result14[0]['SUM(cash_in_pledge_end)']; ?></td>
                  </tr>
                  <?}?>
                </tbody>
                <tr>
