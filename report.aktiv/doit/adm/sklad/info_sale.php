...
if($region == 'Все'){
$data = R::find('tickets',"status = 5 AND salerstatus =1 AND (region = 'Караганда' OR region = 'Кокшетау' OR region = 'Костанай' OR region = 'Павлодар' OR region = 'Нур-Султан') AND datesale BETWEEN :date AND :date1 ",[':date'=>$date1,':date1'=>$date2] );
$data1 = R::find('tickets',"status = 5 AND salerstatus =2 AND (region = 'Караганда' OR region = 'Кокшетау' OR region = 'Костанай' OR region = 'Павлодар' OR region = 'Нур-Султан')  AND datesale BETWEEN :date AND :date1 ",[':date'=>$date1,':date1'=>$date2]); 
$product = R::find('productreport',"datereg BETWEEN :date AND :date1",[':date'=>$date1,':date1'=>$date2] );
$product = R::find('productreport',"salerstatus IS NULL  AND datereg BETWEEN :date AND :date1",[':date'=>$date1,':date1'=>$date2] );
$product1 = R::find('productreport',"salerstatus = 2  AND datereg BETWEEN :date AND :date1",[':date'=>$date1,':date1'=>$date2] );
}          
