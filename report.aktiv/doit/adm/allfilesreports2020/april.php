...
if (isset($_SESSION['logged_user'])) :   //если сущесттвует пользователь
  ...
  if ($_SESSION['logged_user']->status == 3) :
    ...
    $result2 = mysqli_query($connect, " SELECT id, region, SUM(dl),SUM(dm),SUM(dop), SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),
                                          SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv) from reports042020  ");
    ...
    $result = mysqli_query($connect, " SELECT id, region, SUM(dl),SUM(dm),SUM(dop), SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv)
                                      from reports042020  GROUP BY region  ");
    $ss = 0;
    $ss2 = 0;
    $ss3 = 0;
    while ($data1 = mysqli_fetch_array($result)) {
      ...
      while ($data4 = mysqli_fetch_array($result4)) {
        $filial =  $data4['adress'];
        $result5 = mysqli_query($connect, " SELECT auktech,aukshubs,nalvzaloge FROM reports042020 WHERE segdata=(SELECT MAX(segdata) FROM reports042020 WHERE region = '$region' AND adress = '$filial' ) ");
                  ...
                  <table id="example2" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      ...
                      ?>
                        ...
                        <tr>
                          <td>*</td>
                          <td><a href="viewreportregion04.php?region=<?= $data1['region']; ?>"><?= $data1['region']; ?></a></td>
                          <td>
                    ...
                    <tfoot>
                      <?
                      $result2 = mysqli_query($connect, " SELECT id, region, SUM(dl),SUM(dm),SUM(dop), SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv) from reports042020  ");
                      $data2 = mysqli_fetch_array($result2);
                      ?>
