...
if (isset($_SESSION['logged_user'])) :   //если сущесттвует пользователь
  ...
  if ($_SESSION['logged_user']->status == 3) :
    $active_mag = 'active';
    $result22 = mysqli_query($connect, " SELECT SUM(summareal), SUM(summaprihod), SUM(pribl), SUM(summazaden) from sales08  ");
...
?>
    ...
    <div class="content-wrapper">
      ...
      <section class="content-header">
        <h1>
          Отчет МАГАЗИНОВ за Август 2020
        </h1>
        <ol class="breadcrumb">
...
// COUNT(*), id,  data, summazaden, SUM(summazaden), SUM(),,,
                      ...
                      ?>
                        ...
                        <tr>
                          ...
                          <td><a href="viewshop08.php?region=<?= $data1['region']; ?>&shop=<?= $data1['adress']; ?>&from=1"><?= $data1['region']; ?>-<?= $data1['adress']; ?></a></td>
                    ...
                    <tfoot>
                      <?
                      $result2 = mysqli_query($connect, " SELECT SUM(summareal), SUM(summaprihod), SUM(pribl), SUM(summazaden) from sales08  ");
                ...
                <div class="layer">
                  <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                      <?
                      $result = mysqli_query($connect, "SELECT COUNT(*), region, adress, SUM(summareal), SUM(summaprihod), SUM(pribl)
                                                        from sales08 WHERE from_tovar = 2 GROUP BY adress ");
...
// COUNT(*), id,  data, summazaden, SUM(summazaden), SUM(),,,
                      ...
                      ?>
                        ...
                        <tr>
                          <td><i class="fa fa-shopping-cart"></i></td>
                          <td><a href="viewshop08.php?region=<?= $data1['region']; ?>&shop=<?= $data1['adress']; ?>&from=2"><?= $data1['region']; ?>-<?= $data1['adress']; ?></a></td>
                          <td><?= number_format($data1['SUM(summareal)'], 0, '.', ' '); ?></td>
                          <td><?= number_format($data1['SUM(summaprihod)'], 0, '.', ' '); ?></td>
                          <td><?= number_format($data1['SUM(pribl)'], 0, '.', ' '); ?></td>
                          <td><?= $data1['COUNT(*)']; ?></td>
                    ...
                    <tfoot>
                      <?
                      $result2 = mysqli_query($connect, " SELECT SUM(summareal), SUM(summaprihod), SUM(pribl), SUM(summazaden) from sales08 WHERE from_tovar = 2  ");
                      $data2 = mysqli_fetch_array($result2);
