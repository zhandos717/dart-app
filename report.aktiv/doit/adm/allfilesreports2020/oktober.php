  ...
  if ($_SESSION['logged_user']->status == 3) :  ?>
    ...
    <?
      ...
      $result2 = mysqli_query($connect, " SELECT id, region, SUM(dl),SUM(dm),SUM(dop), SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),
                                            SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv) from reports102020  ");
      ...
      $result = mysqli_query($connect, " SELECT id, region, SUM(dl),SUM(dm),SUM(dop), SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv)
                                        from reports102020  GROUP BY region  ");
      $ss = 0;
      $ss2 = 0;
      $ss3 = 0;
                       ...
                       <table class="table table-bordered table-hover" style="white-space:nowrap;">
                         ...
                         <tbody>
                           <?
                            ...
                            ?>
                             <tr>
                               <td><a href="viewreportregion10.php?region=<?= $data1['region']; ?>"> <?= $data1['region']; ?></a></td>
                                 <td>
                              ...
                              <td><?= number_format($data1['SUM(tekrashod)'], 0, '.', ' '); ?></td> -->
                               ...
                               $result12 =$mysqli->query("SELECT SUM(summa_vydachy),SUM(p1),SUM(proc)  FROM tickets WHERE region = '$region1' AND NOT (status = '11' OR status = '1' ) AND dataseg BETWEEN '2020-10-01' AND '2020-10-31' ");
                               ...
                               $result19 =$mysqli->query("SELECT SUM(summa_vydachy),SUM(p1),SUM(proc)  FROM tickets WHERE region = '$region1' AND status = '4' AND datavykup BETWEEN '2020-10-01' AND '2020-10-31' ");
                         ...
                         <tfoot>
                           <?
                            $result2 = mysqli_query($connect, " SELECT id, region, SUM(dl),SUM(dm),SUM(dop), SUM(dk), SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv) from reports102020  ");
                            $data2 = mysqli_fetch_array($result2);
                            ?>
                           <tr style="background: #d3d7df; color: black;">
                             ...
                             $result12 =$mysqli->query("SELECT SUM(summa_vydachy),SUM(p1),SUM(proc)  FROM tickets WHERE NOT (status = '11' OR status = '1' ) AND dataseg BETWEEN '2020-10-01' AND '2020-10-31' ");
                             $data89 = mysqli_fetch_array($result12);

                             $result81 = mysqli_query($connect, " SELECT SUM(summa_vydachy), SUM(cena_pr),SUM(profit) FROM tickets  WHERE status = '5' AND datesale BETWEEN '2020-10-01' AND '2020-10-31'  ");
                             $data81 = mysqli_fetch_array($result81);

                             $result19 = mysqli_query($connect, " SELECT SUM(proc) FROM tickets  WHERE status = '4' AND datavykup BETWEEN '2020-10-01' AND '2020-10-31'  ");
                             $data19 = mysqli_fetch_array($result19);
                   ...
                   <div class="box-body">
                     <div class="table-responsive" >
                       <table class="table table-bordered table-hover" style="white-space:nowrap;">
                         ...
                         <tbody>
                           <?
                            $result = mysqli_query($connect, " SELECT SUM(p1), region FROM tickets WHERE NOT (status = '11' OR status = '1' ) AND dataseg BETWEEN '2020-10-01' AND '2020-10-31'  GROUP BY region  ");

                            while ($data1 = mysqli_fetch_array($result)) {
                              $region = $data1['region'];

                              $result12 =$mysqli->query("SELECT COUNT(*) as count ,SUM(summa_vydachy)  FROM tickets WHERE region = '$region' AND NOT (status = '11' OR status = '1' ) AND dataseg BETWEEN '2020-10-01' AND '2020-10-31' ");
                              $data89 = mysqli_fetch_array($result12);


                              $result19 =$mysqli->query("SELECT SUM(proc) FROM tickets WHERE region = '$region' AND status = '4' AND datavykup BETWEEN '2020-10-01' AND '2020-10-31' ");
                              $data19 = mysqli_fetch_array($result19);
                              ...
                              $result13 =$mysqli->query("SELECT SUM(summa_vydachy) FROM tickets WHERE region = '$region' AND  status = '4' AND dataseg BETWEEN '2020-10-01' AND '2020-10-31' ");
                              ...
                              $result5 = mysqli_query($connect, " SELECT SUM(summa_vydachy) FROM tickets  WHERE region = '$region'  AND status = '2' AND dataseg BETWEEN '2020-10-01' AND '2020-10-31'  ");
                              $data5 = mysqli_fetch_array($result5);

                              $result8 = mysqli_query($connect, " SELECT SUM(summa_vydachy), SUM(cena_pr),SUM(profit) FROM tickets  WHERE region = '$region' AND status = '5' AND datesale BETWEEN '2020-10-01' AND '2020-10-31'  ");
                            ...
                            ?>
                             <tr>
                               <td><a class="btn btn-block bg-olive btn-sm" href="viewreportregion10.php?region=<?if($data1['region'] == 'Нур-султан'){echo "Астана";}else {
                                 echo $data1['region'];
                               };?>"><?=$data1['region']; ?></a></td>
                                 <td>
                         ...
                         <tfoot>
                           ...
                           <tr style="background: #d3d7df; color: black;">
                             <th>Итого (СУММА)</th>
                             <?
                             $result12 =$mysqli->query("SELECT SUM(p1),SUM(summa_vydachy),SUM(cena_pr),COUNT(*) as count FROM tickets WHERE NOT  status = 11 AND NOT  status = 1 AND dataseg BETWEEN '2020-10-01' AND '2020-10-31' ");
                             $data2 = mysqli_fetch_array($result12);

                             $result19 =$mysqli->query("SELECT SUM(proc) FROM tickets WHERE status = '4'  AND datavykup BETWEEN '2020-10-01' AND '2020-10-31' ");
                             $data19  = mysqli_fetch_array($result19);

                             $result12 =$mysqli->query("SELECT SUM(summa_vydachy),SUM(cena_pr),SUM(profit)  FROM tickets WHERE  status = '5'  AND datesale BETWEEN '2020-10-01' AND '2020-10-31' ");
