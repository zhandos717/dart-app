                  ...
                  <table id="example2" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      ...
                      $result = mysqli_query($connect, "SELECT adress,id, region, SUM(stabrashod), SUM(dohod),SUM(tekrashod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv)
                                        FROM reports012020 WHERE `region`='$region' GROUP BY `adress` ");
                      ...
                      ?>
                        ...
                        <tr>
                          <td><a href="viewfilial01.php?region=<?= $region; ?>&adress=<?= $data1['adress']; ?>"><?= $filial; ?></a></td>
                          ...
                          <?
                          $result3 = mysqli_query($connect, " SELECT * FROM reports012020 WHERE reg_date=(SELECT MAX(reg_date) FROM reports012020 WHERE region = '$region' AND adress = '$filial' ) ");
                    ...
                    <tfoot>
                      <tr style="background: #398ebd; color: white;">
                        ...
                        $result2 = mysqli_query($connect, " SELECT id, region, SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv)
                                        from reports012020 WHERE region = '$region' ");
                        ...
                        <?
                        $result4 = mysqli_query($connect, "SELECT *FROM reports012020 WHERE region='$region' GROUP BY adress ");
                        $s = 0;
                        $s2 = 0;
                        $s3 = 0;
                        while ($data4 = mysqli_fetch_array($result4)) {
                          $filial =  $data4['adress'];
                          $result5 = mysqli_query($connect, " SELECT auktech,aukshubs,nalvzaloge FROM reports012020 WHERE reg_date=(SELECT MAX(reg_date) FROM reports012020 WHERE region = '$region' AND adress = '$filial' ) ");
