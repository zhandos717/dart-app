                ...
                <div class="table-responsive">
                  <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                      <?
                      $i = 1;
                      $result = mysqli_query($connect, "SELECT *FROM sales10 WHERE data = '$data_z'  AND region = '$region' AND adress = '$shop' AND fromtovar = '$fromtovar' ");
                      while ($data1 = mysqli_fetch_array($result)) { ?>
                        <tr>
                          <td><?=$i++;?>.</td>
                          <td>
                            <form action="upd_sales10.php" method="post">
                          ...
                          <td><?= $data1['codetovar']; ?></td>
                            <td><? echo  $data1['region'].'/'.$data1['adress']; ?> </td>
                    ...
                    <tfoot>
                      <tr style="background: #d3d7df; color: black;">
                        ...
                        $result2 = mysqli_query($connect, " SELECT id, region, SUM(summaprihod),SUM(predoplata),SUM(summareal),SUM(pribl)
                                        from sales10 WHERE data = '$data_z' AND region = '$region' AND adress = '$shop' AND fromtovar = '$fromtovar' ");
