...
?>
    ...
    $result2 = mysqli_query($connect, " SELECT id, region, SUM(dl),SUM(dm),SUM(dop), SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),
                                        SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv) from reports092020 WHERE `region`='$region'  ");
    ...
    $result4 = mysqli_query($connect, "SELECT *FROM reports092020 WHERE region='$region' GROUP BY adress ");
    $ss = 0;
    $ss2 = 0;
    $ss3 = 0;
    while ($data4 = mysqli_fetch_array($result4)) {
      $filial =  $data4['adress'];
      $result5 = mysqli_query($connect, " SELECT auktech,aukshubs,nalvzaloge FROM reports092020 WHERE segdata=(SELECT MAX(segdata) FROM reports092020 WHERE region = '$region' AND adress = '$filial' ) ");
    ...
    <div class="content-wrapper">
      ...
      <section class="content-header">
        <h1>
          Отчет за Сентябрь 2020, за весь регион - <b><?= $region; ?></b>
                  ...
                  <table id="example2" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      ...
                      $result = mysqli_query($connect, "SELECT adress,id, region, SUM(dl),SUM(dm),SUM(dop), SUM(stabrashod), SUM(dohod),SUM(tekrashod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv)
                                        FROM reports092020 WHERE `region`='$region' GROUP BY `adress` ");
                      ...
                      ?>
                        ...
                        <tr>
                          <td><a href="viewfilial09.php?region=<?= $region; ?>&adress=<?= $data1['adress']; ?>"><?= $filial; ?></a></td>
                          ...
                          <?
                          $result3 = mysqli_query($connect, " SELECT * FROM reports092020 WHERE segdata=(SELECT MAX(segdata) FROM reports092020 WHERE region = '$region' AND adress = '$filial' ) ");
                    ...
                    <tfoot>
                      <tr style="background: #d3d7df; color: black;">
                        ...
                        $result2 = mysqli_query($connect, " SELECT id, region, SUM(dl),SUM(dm),SUM(dop), SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv)
                                        from reports092020 WHERE region = '$region' ");
