...
if (isset($_SESSION['logged_user'])) :   //если сущесттвует пользователь
  ...
  if ($_SESSION['logged_user']->status == 3) :
    ...
    $result2 = mysqli_query($connect, " SELECT id, region, SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),
                                        SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv) from reports022020  ");
    ...
    $result = mysqli_query($connect, " SELECT id, region, SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv)
                                    from reports022020  GROUP BY region  ");
    $ss = 0;
    $ss2 = 0;
    $ss3 = 0;
                  ...
                  <table id="example2" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      ...
                      ?>
                        <tr>
                          <td>*</td>
                          <td><a href="viewreportregion02.php?region=<?= $data1['region']; ?>"><?= $data1['region']; ?></a></td>
                          <td>
                    ...
                    <tfoot>
                      <?
                      $result2 = mysqli_query($connect, " SELECT id, region, SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv) from reports022020  ");
