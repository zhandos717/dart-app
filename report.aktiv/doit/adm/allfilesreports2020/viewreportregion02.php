...
?>
    ...
    <div class="content-wrapper">
      ...
      <section class="content-header">
        <h1>
          Регионы
          <small><a href="february.php">назад к списку</a></small>
        </h1>
        <ol class="breadcrumb">
                ...
                <div class="layer">
                  <table id="example2" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      ...
                      $result = mysqli_query($connect, "SELECT adress,id, region, SUM(stabrashod), SUM(dohod),SUM(tekrashod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv)
                                        FROM reports022020 WHERE `region`='$region' GROUP BY `adress` ");
                      ...
                      ?>
                        ...
                        <tr>
                          <td><a href="viewfilial02.php?region=<?= $region; ?>&adress=<?= $data1['adress']; ?>"><?= $filial; ?></a></td>
                          ...
                          <?
                          $result3 = mysqli_query($connect, " SELECT * FROM reports022020 WHERE reg_date=(SELECT MAX(reg_date) FROM reports022020 WHERE region = '$region' AND adress = '$filial' ) ");
                    ...
                    <tfoot>
                      <tr style="background: #398ebd; color: white;">
                        ...
                        $result2 = mysqli_query($connect, " SELECT id, region, SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv)
                                        from reports022020 WHERE region = '$region' ");
                        ...
                        <?
                        $result4 = mysqli_query($connect, "SELECT *FROM reports022020 WHERE region='$region' GROUP BY adress ");
                        $s = 0;
                        $s2 = 0;
                        $s3 = 0;
                        while ($data4 = mysqli_fetch_array($result4)) {
                          $filial =  $data4['adress'];
                          $result5 = mysqli_query($connect, " SELECT auktech,aukshubs,nalvzaloge FROM reports022020 WHERE reg_date=(SELECT MAX(reg_date) FROM reports022020 WHERE region = '$region' AND adress = '$filial' ) ");
      ...
      <section class="content">
        ...
        <form action="viewreportregion02.php?region=<?= $region; ?>" method="post">
