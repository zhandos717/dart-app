...
<?
      ...
      $result2 = mysqli_query($connect, " SELECT id, region, SUM(dl),SUM(dm),SUM(dop), SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),
                                          SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv) from reports112020 WHERE `region`='$region'  ");
              ...
              <table class="table table-bordered table-hover" style="white-space:nowrap;" id="datatable-tabletools">
                ...
                <tbody>
                  <?
                   ...
                   $result = mysqli_query($connect, " SELECT id, region,adress, SUM(dl),SUM(dm),SUM(dop), SUM(dk), SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv)
                                                       FROM reports112020 WHERE region = '$region'  GROUP BY adress  ");
                   ...
                   ?>
                  <tr>
                    <td><a href="viewfilial11.php?region=<?= $region; ?>&adress=<?= $data1['adress']; ?>"><?= $data1['adress']; ?></a></td>
                    <td>
                    ...
                    <?
                      ...
                      $result12 =$mysqli->query("SELECT SUM(summa_vydachy),SUM(p1),SUM(proc)  FROM tickets WHERE adressfil = '$adress1' AND NOT (status = '11' OR status = '1' ) AND dataseg BETWEEN '2020-11-01' AND '2020-11-30' ");
                      $data89 = mysqli_fetch_array($result12);
                      $result19 =$mysqli->query("SELECT SUM(proc)  FROM tickets WHERE adressfil = '$adress1' AND status = '4' AND datavykup BETWEEN '2020-11-01' AND '2020-11-30' ");
                      $data19 = mysqli_fetch_array($result19);
                      $result81 = mysqli_query($connect, " SELECT SUM(summa_vydachy), SUM(cena_pr),SUM(profit) FROM tickets  WHERE adressfil = '$adress1' AND status = '5' AND datesale BETWEEN '2020-11-01' AND '2020-11-30'  ");
                 ...
                 //     $result812 = mysqli_query($connect, " SELECT SUM(summa) FROM plancomission  WHERE region = '$region1' AND vid = '2'  ");
                   //   $data451 = mysqli_fetch_array($result812);
                    ...
                    <?
                       $result4 = mysqli_query($connect, "SELECT *FROM reports112020 WHERE adress='$adress' GROUP BY adress ");
                       $s = 0;
                       $s2 = 0;
                       $s3 = 0;
                       while ($data4 = mysqli_fetch_array($result4)) {
                ...
                <tfoot>
                  ...
                  $result2 = mysqli_query($connect, " SELECT id, region, SUM(dl),SUM(dm),SUM(dop), SUM(dk), SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv)
                                                      FROM reports112020 WHERE region='$region'  ");
                   $data2 = mysqli_fetch_array($result2);
                   ?>
                  <tr style="background: #d3d7df; color: black;">
                    ...
                    $region1 = $region;
                    if($region1 == 'Астана'){$region1= 'Нур-султан';};
                    $result12 =$mysqli->query("SELECT SUM(summa_vydachy),SUM(p1)  FROM tickets WHERE region='$region1' AND NOT (status = '11' OR status = '1' ) AND dataseg BETWEEN '2020-11-01' AND '2020-11-30' ");
                    $data89 = mysqli_fetch_array($result12);
                    $result19 =$mysqli->query("SELECT  SUM(proc)  FROM tickets WHERE region='$region1' AND status = '4'  AND datavykup BETWEEN '2020-11-01' AND '2020-11-30' ");
          ...
          <div class="box-body">
            <div class="table-responsive">
              <table class="table table-bordered table-hover" style="white-space:nowrap;">
                ...
                <tbody>
                  <?
                   $result = mysqli_query($connect, " SELECT SUM(p1), region,adressfil FROM tickets WHERE NOT (status = '11' OR status = '1' ) AND dataseg BETWEEN '2020-11-01' AND '2020-11-30' AND region = '$region1'  GROUP BY adressfil  ");

                   while ($data1 = mysqli_fetch_array($result)) {
                     $adress = $data1['adressfil'];
                     $region = $data1['region'];

                     $result19 = mysqli_query($connect, " SELECT SUM(proc) FROM tickets WHERE status = '4' AND adressfil = '$adress' AND datavykup BETWEEN '2020-11-01' AND '2020-11-30'  ");
                     $data19 = mysqli_fetch_array($result19);

                     $result12 =$mysqli->query("SELECT COUNT(*) as count ,SUM(summa_vydachy)  FROM tickets WHERE adressfil = '$adress' AND NOT (status = '11' OR status = '1' ) AND dataseg BETWEEN '2020-11-01' AND '2020-11-30' ");
                     $data89 = mysqli_fetch_array($result12);
                     ...
                     $result13 =$mysqli->query("SELECT SUM(summa_vydachy) FROM tickets WHERE adressfil = '$adress' AND  status = '4' AND dataseg BETWEEN '2020-11-01' AND '2020-11-30' ");
                     ...
                     $result5 = mysqli_query($connect, " SELECT SUM(summa_vydachy) FROM tickets  WHERE adressfil = '$adress'  AND status = '2' AND dataseg BETWEEN '2020-11-01' AND '2020-11-30'  ");
                     $data5 = mysqli_fetch_array($result5);

                     $result8 = mysqli_query($connect, " SELECT SUM(summa_vydachy), SUM(cena_pr),SUM(profit) FROM tickets  WHERE adressfil = '$adress' AND status = '5' AND datesale BETWEEN '2020-11-01' AND '2020-11-30'  ");
                ...
                <tfoot>
                  ...
                  <tr style="background: #d3d7df; color: black;">
                    <th>Итого (СУММА)</th>
                    <?
                    $result12 =$mysqli->query("SELECT SUM(p1), SUM(summa_vydachy),SUM(cena_pr),COUNT(*) as count FROM tickets WHERE  NOT  status = 11 AND NOT  status = 1 AND region = '$region' AND dataseg BETWEEN '2020-11-01' AND '2020-11-30'  ");
                    $data2 = mysqli_fetch_array($result12);

                    $result19 =$mysqli->query("SELECT SUM(proc)  FROM tickets WHERE status = 4 AND region = '$region' AND datavykup BETWEEN '2020-11-01' AND '2020-11-30'  ");
                    $data19 = mysqli_fetch_array($result19);

                     $result12 =$mysqli->query("SELECT SUM(summa_vydachy),SUM(cena_pr),SUM(profit)  FROM tickets WHERE  status = '5'  AND datesale BETWEEN '2020-11-01' AND '2020-11-30' AND region = '$region'  ");
