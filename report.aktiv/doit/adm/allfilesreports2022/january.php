                ...
                <table class="table table-bordered table-hover" style="white-space:nowrap;" id="datatable-tabletools">
                  ...
                  <tbody>
                    ...
                    while ($data1 = mysqli_fetch_array($result)) {
                      ...
                      $sale12 = $sales[0];
                      $reg = ['Нур-Султан', 'Темиртау', 'Костанай', 'Кокшетау', 'Павлодар', 'Караганда', 'Петропавловск'];
                      ...
                      if (in_array($region, $reg)) {
                        ...
                        $da = R::findOne('comision2region', "data = ? AND region = ? ", [$day, $region]);
                        $tbs = $da['comis'] + $da['proc'] + ($da['cena_pr'] - $da['sale']) + $da['profit'] - $da['SUM(summa)'] + ($da['price'] - $da['purchaseamount']) - $ras;
                        $tbs = percent($tbs, 3);
                      };


                        $datar = R::getCol("SELECT SUM(summarf) FROM rashodfillial WHERE region = '$region'AND datarashoda BETWEEN '$data_begin' AND '$data_end'");

                        $obshirashodregiona = $datar[0];

                        $rashodporegionam += $obshirashodregiona;
                        ?>
                      <tr>
                        ...
                        <td style="background: #00c2f0; color: black;"><strong><?= number_format(percent($data1['SUM(dohod)'] - $obshirashodregiona, 20), 0, '.', ' '); ?></strong></td>
                        <th>
                          <?= number_format($tbs, 0, '.', ' ');
                          $tbsTotal += $tbs;  ?>
                        </th>
                        ...
                        <th class="success"><?= number_format($obs + percent($data1['SUM(dohod)'] - $obshirashodregiona, 20) + $tbs, 0, '.', ' '); ?>
                        ...
                        <?
                        $result4 = R::getCol("SELECT  adress FROM reports012022 WHERE region='$region' GROUP BY adress ");
                        $s = 0;
                        $s2 = 0;
                        $s3 = 0;
                        foreach ($result4 as $filial) {
                          $nal = R::findOne('reports012022', 'adress =? ORDER BY data DESC LIMIT 1', [$filial]);
                          $s += $nal['auktech'];
                          $s2 += $nal['aukshubs'];
                          $s3 += $nal['nalvzaloge'];
                        }
                  ...
                  <tfoot>
                    <? $result2 = mysqli_query($connect, " SELECT id, region, SUM(dl),SUM(dm),SUM(dop), SUM(dk), SUM(dohod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),SUM(chv) from reports012022  ");
                    ...
                    <tr style="background: #d3d7df; color: black;">
                      ...
                      <th style="background: #00c2f0; color: black;"><strong><?= number_format(percent($data2['SUM(dohod)'] - $rashodporegionam, 20), 0, '.', ' '); ?></strong></th>
                      <th>
                        <?= number_format($tbsTotal, 0, '.', ' '); ?>
                      </th>
                      <th class="danger"><?= number_format($obsTotal, 0, '.', ' '); ?></th>

                      <th class="success"><?= number_format($obsTotal + percent($data2['SUM(dohod)'] - $rashodporegionam, 20) + $tbsTotal, 0, '.', ' '); ?></th>
              ...
              <div class="table-responsive">
                <table class="table table-bordered table-hover" id="tabletools">
                  ...
                  <tbody>
                    ...
                    ?>
                      <tr>
                        <td><a class="btn btn-block bg-olive" href="viewreportregion01.php?region=<?= $data1['region']; ?>"><?= $data1['region']; ?></a></td>
                        <td>
