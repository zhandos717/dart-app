                ...
                <table class="table table-bordered table-hover" id="datatable-tabletools" style="white-space:nowrap;">
                  ...
                  <tbody>
                    ...
                    $result = R::getAll(" SELECT id, region,adress, SUM(dl),SUM(dm),SUM(dop), SUM(dk), SUM(dohod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv)
                                                       FROM reports122021 WHERE region = '$region'  GROUP BY adress  ");
                    foreach ($result as $data1) {
                      ...
                      $resultsr = mysqli_query($connect, "SELECT SUM(summarf) FROM `rashodfillial122021` WHERE region = '$region'
                                                     ");
                      $datasr = mysqli_fetch_array($resultsr);
                      $obshirashodregiona = $datasr['SUM(summarf)'];


                      $nal = R::findOne('reports122021', 'adress =? ORDER BY data DESC LIMIT 1', [$adress]);
                      $table = R::getCol("SELECT SUM(tekrashod),adress FROM comisstest WHERE adress=? AND data BETWEEN '$data_begin' AND '$data_end' ", [$adress]);
                      $ras = $table[0];
                      $res = R::getRow("SELECT * FROM reports122021 WHERE adress = '$adress' ORDER BY reg_date DESC LIMIT 1");
                    ...
                    ?>
                      <tr>
                        <td><a href="viewfilial12.php?region=<?= $region; ?>&adress=<?= $data1['adress']; ?>"><?= $data1['adress']; ?></a></td>
                        <td>
                  ...
                  <tfoot>
                    <tr class="bg-gray">
                      ...
                      <th><?= number_format($obshirashodregiona, 0, '.', ' '); ?></th>
