...
if ($_SESSION['logged_user']->status == 3) :
  ...
  $data_begin = '2021-11-01'; //Дата начало
  $data_end   = '2021-11-30'; //Дата конец
  $month = 11;
  $day =  $data_end;
                ...
                <table class="table table-bordered table-hover" id="datatable-tabletools" style="white-space:nowrap;">
                  ...
                  <tbody>
                    ...
                    foreach ($result as $data1) {
                      $region =  $data1['region'];
                      $adress =  $data1['adress'];
                      $nal = R::findOne($table, 'adress =? ORDER BY data DESC LIMIT 1', [$adress]);
                      $comisstest = R::getCol("SELECT SUM(tekrashod),adress FROM comisstest WHERE adress=? AND data BETWEEN '$data_begin' AND '$data_end' ", [$adress]);
                      $ras = $comisstest[0];
                      $res = R::getRow("SELECT * FROM $table WHERE adress = '$adress' ORDER BY reg_date DESC LIMIT 1");
