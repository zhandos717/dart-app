...
if($_SESSION['logged_user']->status == 3) :
    ...
    function percent_comiss($number) {
      ...
      $result4 = mysqli_query($connect, "SELECT *FROM reports022021 WHERE region='$region' GROUP BY adress ");
      $ss = 0;
      $ss2 = 0;
      $ss3 = 0;
      while ($data4 = mysqli_fetch_array($result4)) {
        $filial =  $data4['adress'];
        $result5 = mysqli_query($connect, " SELECT auktech,aukshubs,nalvzaloge FROM reports022021 WHERE segdata=(SELECT MAX(segdata) FROM reports022021 WHERE region = '$region' AND adress = '$filial' ) ");
      ...
      foreach ($filOne as $find) {
                 ...
                 if(!empty($arr['summa_vydachy'])){
                  $test=R::findOne('comision2','adress=? AND data =? ',[$find['filial'],$day]);
                  if(empty($test)){
      ...
      $result2 = mysqli_query($connect, " SELECT id, region, SUM(dl),SUM(dm),SUM(dop), SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),
                                          SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv) from reports022021 WHERE `region`='$region'  ");
                ...
                <tbody>
                  <?
                   $result = mysqli_query($connect, " SELECT id, region,adress, SUM(dl),SUM(dm),SUM(dop), SUM(dk), SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv)
                                                       FROM reports022021 WHERE region = '$region'  GROUP BY adress  ");
                   while ($data1 = mysqli_fetch_array($result)) {
                    ...
                    $ras =$table[0];
                    $res = R::getRow("SELECT * FROM reports022021 WHERE adress = '$adress' ORDER BY reg_date DESC LIMIT 1");
                    ...
                    ?>
                  <tr>
                    <td><a href="viewfilial02.php?region=<?= $region; ?>&adress=<?= $data1['adress']; ?>"><?= $data1['adress']; ?></a></td>
                    <td>
                    ...
                    <?
                       $result4 = mysqli_query($connect, "SELECT *FROM reports022021 WHERE adress='$adress' GROUP BY adress ");
                       $s = 0;
                       $s2 = 0;
                       $s3 = 0;
                       while ($data4 = mysqli_fetch_array($result4)) {
              ...
              <table class="table table-bordered table-hover">
                ...
                <tbody>
                  <?  foreach($comis2 as $data2) {
                      ...
                      $table =R::getCol('SELECT SUM(tekrashod) FROM comisstest WHERE adress=? AND data =?',[$data2['adress'],$day]);
                      $ras =$table[0];
          ...
          <div class="box-body">
            <div class="table-responsive">
              <table class="table table-bordered table-hover" style="white-space:nowrap;">
                ...
                <tbody>
                  <?
                   $result = mysqli_query($connect, " SELECT SUM(p1), region,adressfil FROM tickets WHERE NOT (status = '11' OR status = '1' ) AND dataseg BETWEEN '2021-02-01' AND '2021-02-28' AND region = '$region'  GROUP BY adressfil  ");
                  while ($data1 = mysqli_fetch_array($result)) {
                    ...
                    $table =R::getCol("SELECT SUM(tekrashod) FROM comisstest WHERE adress=? AND data BETWEEN '2021-02-01' AND '2021-02-28' " ,[$adress]);
                    $ras =$table[0];

                    $chistaya = $data1['SUM(p1)']+$data19['SUM(proc)']+($data8['SUM(cena_pr)']-$data8['SUM(summa_vydachy)'])+$data8['SUM(profit)']- $ras;
                    $chistaya = percent_comiss($chistaya);
                   ?>
                  <tr>
