              ...
              <div class="">
                <table id="datatable-tabletools" class="table table-bordered table-hover">
                  ...
                  <tbody>
                    ...
                    $result = R::findAll('reports122021', 'region = ? AND adress = ? ORDER BY data', [$region, $adress]);
                    ...
                    foreach ($result as $data1) {
                      ...
                      $datarashoda = $data1['data'];
                      $resultr = mysqli_query($connect, "SELECT SUM(summarf) FROM `rashodfillial122021` WHERE region = '$region' AND adress = '$adress'
                                                       AND datarashoda = '$datarashoda'");
                      $datar = mysqli_fetch_array($resultr);

                      $resultsr = mysqli_query($connect, "SELECT SUM(summarf) FROM `rashodfillial122021` WHERE region = '$region' AND adress = '$adress'
                                                       ");
                      $datasr = mysqli_fetch_array($resultsr);
                      $summarashodov = $datasr['SUM(summarf)'];// Итоговая сумма расходов
                      $pribl = $data1['dohod'] - $datar['SUM(summarf)'];
                      // $pribl = $data1['dohod'] - $data1['stabrashod'] - $data1['tekrashod'];

                      $resultc = mysqli_query($connect, "SELECT comments FROM `rashodfillial122021` WHERE region = '$region' AND adress = '$adress'
                                                       AND datarashoda = '$datarashoda'");
                      ...
                      $pr = $data1['dohod'] - $datar['SUM(summarf)']; // Итог прибыли
                      // $stabrashod += $data1['stabrashod'];
                      // $tekrashod += $data1['tekrashod'];

                      $chistaya += $chistpribl;

                      $rashod += $datar['SUM(summarf)']; // Итог расходов
                    ...
                    ?>
                      ...
                      <tr style="white-space:nowrap;">
                        <td><a href="look12.php?id=<?= $data1['id']; ?>" class="btn btn-warning"><i class="fa fa-pencil"></i></a></td>
                        ...
                        <td><?= number_format($datar['SUM(summarf)'], 0, '.', ' '); ?></td>
                        <td><i><?= $datac['comments']; ?></i><br></td>
                  ...
                  <tfoot>
                    <tr class="bg-olive">
                      ...
                      <td><?= number_format($summarashodov, 0, '.', ' '); ?></td>
                      <td></td>
