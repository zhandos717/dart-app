...
if (isset($_SESSION['logged_user'])) :   //если сущесттвует пользователь
  ...
  if ($_SESSION['logged_user']->status == 3) :
    $active_mag = 'active';
    $result22 = mysqli_query($connect, " SELECT SUM(summareal), SUM(summaprihod), SUM(pribl), SUM(summazaden) from sales04  ");
...
?>
    ...
    <div class="content-wrapper">
      ...
      <section class="content-header">
        <h1>
          Отчет МАГАЗИНОВ за АПРЕЛЬ 2020
        </h1>
        <ol class="breadcrumb">
                  ...
                  <table  class="table table-bordered table-hover">
                    <thead>
                      <tr style="background: #398ebd; color: white;">
                        ...
                        <th>Приход.Сумма</th>
                        <th>ПРИБЫЛЬ</th>
                        <th>Колличество <br /> проданных<br />товаров</p>
                        </th>
                        <th>ПРИБЫЛЬ %</th>
                        <th>АУКЦИОНИСТ +/-</th>
                    ...
                    <tbody>
                      <?
                      $result = mysqli_query($connect, "SELECT COUNT(*), id, region,adress, data, summazaden, SUM(summazaden), SUM(summaprihod),SUM(summakredit),SUM(summareal),SUM(pribl)
                         from sales04 GROUP BY adress ");
                      $sc = 0;
                      ...
                      ?>
                        ...
                        <tr>
                          ...
                          <td><a href="viewshop04.php?region=<?= $data1['region']; ?>&shop=<?= $data1['adress']; ?>"><?= $data1['region']; ?>-<?= $data1['adress']; ?></a></td>
                    ...
                    <tfoot>
                      <?
                      $result2 = mysqli_query($connect, " SELECT SUM(summareal), SUM(summaprihod), SUM(pribl), SUM(summazaden) from sales04  ");
