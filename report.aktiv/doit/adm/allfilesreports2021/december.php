                ...
                <table class="table table-bordered table-hover" style="white-space:nowrap;" id="datatable-tabletools">
                  ...
                  <tbody>
                    ...
                    while ($data1 = mysqli_fetch_array($result)) {
                      ...
                      $resultr = mysqli_query($connect, "SELECT SUM(summarf) FROM `rashodfillial122021` WHERE region = '$regionobsh' ");
                      $datar = mysqli_fetch_array($resultr);
                      $obshirashodregiona = $datar['SUM(summarf)'];

                      $resultsr = mysqli_query($connect, "SELECT SUM(summarf) FROM `rashodfillial122021`  ");
                      ...
                      <tr>
                        <td><a href="viewreportregion12.php?region=<?= $region  ?>"> <?= $region  ?></a></td>
                        <td>
                        ...
                        <?
                        $chistaya = percent($data1['SUM(dohod)'] - $obshirashodregiona, 20); // чистая прибыль  = за минусом 20 процентов
                        ?>
                        <td>
                        ...
                        <?
                        $result4 = R::getCol("SELECT  adress FROM reports122021 WHERE region='$region' GROUP BY adress ");
                        ...
                        foreach ($result4 as $filial) {
                          $nal = R::findOne('reports122021', 'adress =? ORDER BY data DESC LIMIT 1', [$filial]);
                          $s += $nal['auktech'];
                          $s2 += $nal['aukshubs'];
                          $s3 += $nal['nalvzaloge'];
                          // $result5 = mysqli_query($connect, " SELECT auktech,aukshubs,nalvzaloge FROM reports122021 WHERE segdata=(SELECT MAX(segdata) FROM reports122021 WHERE region = '$region' AND adress = '$filial' ) ");
                  ...
                  <tfoot>
                    <? $result2 = mysqli_query($connect, " SELECT id, region, SUM(dl),SUM(dm),SUM(dop), SUM(dk), SUM(dohod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv) from reports122021  ");
                    ...
                    <th><?= number_format($data2['SUM(tekrashod)'], 0, '.', ' '); ?></th> -->
                      <?
                      $chistaya = percent($data2['SUM(dohod)'] - $rashodporegionam, 20);                                                   // чистая прибыль  = за минусом 20 процентов
                      ?>
