...
if($_SESSION['logged_user']->status == 3) :
    $region = $_GET['region'];
      ...
      include "menu.php";
          ...
          $data_begin = '2021-04-01'; //Дата начало
          $data_end   = '2021-04-30'; //Дата конец

        if($region != 'Караганда' AND  $region != 'Нур-Султан'){
          $comis2 = R::findAll('comision2',"region=:region AND day =:data GROUP BY adress",[ ':region'=>$region, ':data'=>$data_end]);
        };
        ?>
                ...
                <tbody>
                  <?
                         $result = R::getAll(" SELECT id, region,adress, SUM(dl),SUM(dm),SUM(dop), SUM(dk), SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv)
                                                             FROM reports042021 WHERE region = '$region'  GROUP BY adress  ");
                         foreach ($result as $data1){
                          $region =  $data1['region'];
                          $adress=  $data1['adress'];
                          $nal = R::findOne('reports042021','adress =? ORDER BY data DESC LIMIT 1',[$adress]);
                          $table =R::getCol("SELECT SUM(tekrashod),adress FROM comisstest WHERE adress=? AND data BETWEEN '$data_begin' AND '$data_end' ",[$adress]);
                          $ras =$table[0];
                          $res = R::getRow("SELECT * FROM reports042021 WHERE adress = '$adress' ORDER BY reg_date DESC LIMIT 1");
                          ...
                          if(!empty($comis2)){
                              $tbs_reg = R::findOne('comision2','adress=? AND day = ? ',[$adress1,$data_end]);
                          };
                          if(!empty($tbs_reg)){
                          ...
                          ?>
                  <tr>
                    <td><a href="viewfilial04.php?region=<?= $region; ?>&adress=<?= $data1['adress']; ?>"><?= $data1['adress']; ?></a></td>
                    <td>
              ...
              <table class="table table-bordered table-hover">
                ...
                <tbody>
                  <?  foreach($comis2 as $data2) {
                    ...
                    $table = R::getCol("SELECT SUM(tekrashod),adress FROM comisstest WHERE adress=? AND data BETWEEN '$data_begin' AND '$data_end' ", [$data2['adress']]);
                    $ras = $table[0];
                    ###########################
                            $dohod = $data2['comis'] + $data2['proc'] + ($data2['cena_pr'] - $data2['sale']) + $data2['profit']-$ras;
                            $chistaya_tbs = percent_comiss($dohod);
                            ?>
          ...
          <div class="box-body">
            <div class="table-responsive">
              <table class="table table-bordered table-hover" style="white-space:nowrap;">
                ...
                <tbody>
                  <?
                $result = R::getAll("SELECT COUNT(*) as count , SUM(p1), adressfil FROM tickets WHERE NOT (status = '11' OR status = '1' ) AND region = '$region'  AND dataseg BETWEEN '$data_begin' AND '$data_end'  GROUP BY adressfil  ");
                foreach($result as $data1) {
                ##################### Регион
                ...
                $auctioneer = R::getAll("SELECT auctioneer_fur, auctioneer_teh, cash_in_pledge_end  FROM repotscom WHERE kassa = 'Касса 1' AND adress = :adress AND datereport = :datereport  GROUP BY adress ",[':adress'=>$adress,':datereport'=>$data_end]);
                ...
                $chistaya = percent_comiss($data1['SUM(p1)'] + $data19['SUM(proc)'] + ($data8['SUM(cena_pr)'] - $data8['SUM(summa_vydachy)']) - $ras);
                ?>
                  <tr>
                    ...
                    <td class="success">
                      <? echo number_format($auctioneer['auctioneer_teh'], 0, '.', ' ');
                        $tehnica_obs += $auctioneer['auctioneer_teh']; ?>
                    </td>
                    <td class="warning">
                      <? echo number_format($auctioneer['auctioneer_fur'], 0, '.', ' ');
                          $shuby_obs +=$auctioneer['auctioneer_fur']; ?>
                    </td>
                    <td class="danger">
                      <? echo number_format($auctioneer['cash_in_pledge_end'], 0, '.', ' ');
                         $nal_obs += $auctioneer['cash_in_pledge_end'];
                        ?>
                    </td>
                  </tr>
                  <?}?>
