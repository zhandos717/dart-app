...
if (isset($_SESSION['logged_user'])) :   //если сущесттвует пользователь
  ...
  if ($_SESSION['logged_user']->status == 3) :
    ...
    $result22 = mysqli_query($connect, " SELECT SUM(summareal), SUM(summaprihod), SUM(pribl), SUM(summazaden) from sales12 WHERE statustovar IS NULL ");
...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            Отчет МАГАЗИНОВ за Декабрь <?= date('Y'); ?>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        <div class="row">
            <div class="col-lg-3 col-xs-6">
                ...
                <div class="small-box bg-aqua">
                    ...
                    <a href="a_report.php?id=6" class="small-box-footer"> <i class="fa fa-arrow-circle-right"></i></a>
                </div>
            </div><!-- ./col -->
            <div class="col-lg-3 col-xs-6">
                <!-- small box -->
        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="box">
                    <div class="box-body">
                      ...
                      FROM sales12 WHERE statustovar IS NULL GROUP BY adress "); // COUNT(*), id,  data, summazaden, SUM(summazaden), SUM(),,,
                      $sc = 0;
                      while ($data1 = mysqli_fetch_array($result)) {
                        $sc = $sc + $data1['COUNT(*)'];
                        $region = $data1['region'];
                        $adress = $data1['adress'];
                        $resultpl = mysqli_query($connect, "SELECT * FROM magplan WHERE month =  12  AND region ='$region' AND adress = '$adress'");
                        ...
                        $procent = ($procent2*100)/$planden1;?>
                                    <tr>
                                        ...
                                        <td><a href="viewshop12.php?region=<?= $data1['region']; ?>&shop=<?= $data1['adress']; ?>&from=1"><?= $data1['region']; ?>-<?= $data1['adress']; ?></a></td>
                                ...
                                <tfoot>
                                    <?
                                                $result2 = mysqli_query($connect, " SELECT SUM(summareal), SUM(summaprihod), SUM(pribl), SUM(summazaden) from sales12 WHERE statustovar IS NULL  ");
                                                $data2 = mysqli_fetch_array($result2);
                                                $aukminus = $data2['SUM(summareal)'] - $data2['SUM(summazaden)'];

                                                $resultplan = mysqli_query($connect, "SELECT SUM(plan) FROM magplan WHERE month =  12 ");
                    ...
                    <div class="box-body">
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    <?
                                $result = mysqli_query($connect, "SELECT COUNT(*), region, adress, SUM(summareal), SUM(summaprihod), SUM(pribl) from sales12 WHERE fromtovar = 1 AND statustovar IS NULL  GROUP BY adress ");// COUNT(*), id,  data, summazaden, SUM(summazaden), SUM(),,,
                                $sc = 0;
                                while ($data1 = mysqli_fetch_array($result)) {
                                  $sc = $sc + $data1['COUNT(*)'];
                                ?>
                                    <tr>
                                        <td><i class="fa fa-shopping-cart"></i></td>
                                        <td><a href="viewshop12.php?region=<?= $data1['region']; ?>&shop=<?= $data1['adress']; ?>&from=2"><?= $data1['region']; ?>-<?= $data1['adress']; ?></a></td>
                                        <td><?= number_format($data1['SUM(summareal)'], 0, '.', ' '); ?></td>
                                        <td><?= number_format($data1['SUM(summaprihod)'], 0, '.', ' '); ?></td>
                                        <td><?= number_format($data1['SUM(pribl)'], 0, '.', ' '); ?></td>
                                        <td><?= $data1['COUNT(*)']; ?></td>
                                ...
                                <tfoot>
                                    <?
                                    $result2 = mysqli_query($connect, " SELECT SUM(summareal), SUM(summaprihod), SUM(pribl), SUM(summazaden) from sales12 WHERE fromtovar = 1 AND statustovar IS NULL  ");
                                  $data2 = mysqli_fetch_array($result2);
                                  $aukminus = $data2['SUM(summareal)'] - $data2['SUM(summazaden)'];
                                ?>
                                    <tr style="background: #d3d7df; color: black;">
                    ...
                    <div class="box-body">
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    <?
                      $result = mysqli_query($connect, "SELECT COUNT(*), region, adress, SUM(summareal), SUM(summaprihod), SUM(pribl) from sales12 WHERE fromtovar = 2 AND statustovar IS NULL  GROUP BY adress ");// COUNT(*), id,  data, summazaden, SUM(summazaden), SUM(),,,
                      $sc = 0;
                      while ($data1 = mysqli_fetch_array($result)) {
                        $sc = $sc + $data1['COUNT(*)'];
                      ?>
                                ...
                                <tfoot>
                                    <?
                                    $result2 = mysqli_query($connect, " SELECT SUM(summareal), SUM(summaprihod), SUM(pribl), SUM(summazaden) from sales12 WHERE fromtovar = 2  AND statustovar IS NULL ");
                        $data2 = mysqli_fetch_array($result2);
                        $aukminus = $data2['SUM(summareal)'] - $data2['SUM(summazaden)'];
                      ?>
                                    <tr style="background: #d3d7df; color: black;">
            ...
            <div class="col-xs-12">
                <div class="box collapsed-box">
                    <div class="box-header">
                        <h4> <a href="accses.php" class="btn  bg-olive">Аксессуары</a> <i data-widget="collapse" class="btn bg-red fa fa-tag"></i> </h4>
                        <div class="box-tools pull-right">
                            <a type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-plus"></i></a>
                            <!-- <button type="button" class="btn btn-box-tool" data-widget="remove"><i class="fa fa-remove"></i></button> -->
                        </div>
                    </div>
                    <div class="box-body">
            ...
            <div class='col-xs-12'>
                ...
                from sales12 WHERE statustovar  IS NULL GROUP BY saler  ");
                $i = 1;
                while($data1 = mysqli_fetch_array($result)){
                  ?>
                                <tr>
