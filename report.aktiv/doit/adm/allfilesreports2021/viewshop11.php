          ...
          <div class="col-md-12">
             <div class="box box-primary">
               <div class="box-header with-border">
                 ...
                 <div class="box-body">
                   <div class="row">
                   <form action="filtr_shop11.php" method="POST">
                     ...
                     <div class="col-lg-2 col-md-2 col-sm-2">
                       <div class="input-group">
                              ...
                              <input type="date" class="form-control" style="width: 16rem;"  name="date1" min="2020-11-01" max="2020-11-30" required>
                        </div>
                        <!-- /input-group -->
                     </div>
                      <div class="col-lg-2 col-md-2 col-sm-2">
                       <div class="input-group">
                              ...
                              <input type="date" class="form-control" style="width: 16rem;" name="date2" min="2020-11-01" max="2020-11-30">
                        </div>
                        <!-- /input-group -->
                     </div>
                               ...
                               $result = mysqli_query($connect, "SELECT *FROM sales11 WHERE region = '$region' AND adress = '$shop' AND fromtovar = '$fromtovar' GROUP BY saler");
...
<!--
                  ...
                  <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                      ...
                      $result = mysqli_query($connect, "SELECT data, SUM(summareal), SUM(summazaden), SUM(pribl), COUNT(*), SUM(summaprihod)
                                        from sales11  WHERE region = '$region' AND adress = '$shop' AND fromtovar = '1' AND statustovar IS NULL GROUP BY data ");
                    while ($data1 = mysqli_fetch_array($result)) {
                      ?>
                        <tr>
                          <td><a href="detail11.php?region=<?= $region; ?>&shop=<?= $shop; ?>&data_z=<?= $data1['data']; ?>&from=1"><?= date("d.m.Y", strtotime($data1['data'])); ?></a></td>
                    ...
                    <tfoot>
                      <tr style="background: #398ebd; color: white;">
                        ...
                        $result2 = mysqli_query($connect, " SELECT SUM(summareal), SUM(summazaden), SUM(pribl), SUM(summaprihod), COUNT(*)
                                        from sales11  WHERE region = '$region' AND adress = '$shop' AND statustovar IS NULL  AND fromtovar = '1' ");
                  ...
                  <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                      ...
                      $result = mysqli_query($connect, "SELECT data, SUM(summareal), SUM(summazaden), SUM(pribl),SUM(summaprihod), COUNT(*)
                                        from sales11  WHERE region = '$region' AND adress = '$shop' AND fromtovar = '2' AND statustovar IS NULL  GROUP BY data ");
                      $sc = 0;
                      while ($data1 = mysqli_fetch_array($result)) {
                        $sc = $sc + $data1['COUNT(*)'];
                      ?>
                        <tr>
                          <td><a href="detail11.php?region=<?= $region; ?>&shop=<?= $shop; ?>&data_z=<?= $data1['data']; ?>&from=2"><?= date("d.m.Y", strtotime($data1['data'])); ?></a></td>
                          <th><?= number_format($data1['SUM(summareal)'], 0, '.', ' '); ?></th>
                          <th><?= number_format($data1['SUM(summaprihod)'], 0, '.', ' '); ?></th>
                          <th><?= number_format($data1['SUM(pribl)'], 0, '.', ' '); ?></th>
                          <th><?= $data1['COUNT(*)']; ?></th>
                    ...
                    <tfoot>
                      <tr style="background: #398ebd; color: white;">
                        ...
                        $result2 = mysqli_query($connect, " SELECT SUM(summareal), SUM(summazaden), SUM(pribl),SUM(summaprihod)
                                        from sales11  WHERE region = '$region' AND adress = '$shop' AND statustovar IS NULL AND fromtovar = '2' ");
                        $data22 = mysqli_fetch_array($result2);
                        $aukminus = $data22['SUM(summareal)'] - $data22['SUM(summazaden)'];
                        ?>
                        <th><strong>ИТОГО </strong></th>
                        <th><strong><?= number_format($data22['SUM(summareal)'], 0, '.', ' '); ?></strong></th>
                        <th><strong><?= number_format($data22['SUM(summaprihod)'], 0, '.', ' '); ?></strong></th>
                        <th><strong><?= number_format($data22['SUM(pribl)'], 0, '.', ' '); ?></strong></th>
                        <th><?= $sc; ?></th>
                      </tr>
                    </tfoot>
                  </table>
                  ...
                  <table class="table table-bordered table-hover" id="datatable-tabletools" >
                    ...
                    <tbody>
                      ...
                      $result = mysqli_query($connect, "SELECT regionlombard, adresslombard, SUM(summareal), SUM(summazaden), SUM(pribl), COUNT(*), SUM(summaprihod)
                                        from sales11  WHERE region = '$region' AND adress = '$shop' AND statustovar IS NULL GROUP BY adresslombard  ");
                      while ($data1 = mysqli_fetch_array($result)) {
                        ?>
                        <tr>
                    ...
                    <tfoot>
                      <tr class="danger">
                        <?
                        $result2 = mysqli_query($connect, " SELECT SUM(summareal), SUM(summazaden), SUM(pribl), SUM(summaprihod), COUNT(*)
                                        from sales11  WHERE region = '$region' AND adress = '$shop' AND statustovar IS NULL  ");
                        $data2 = mysqli_fetch_array($result2);
                        $aukminus = $data2['SUM(summareal)'] - $data2['SUM(summazaden)'];
                        ?>
        ...
        <div class="box">
                ...
                $i = 1;
                while($data1 = mysqli_fetch_array($result)){
                  $procent = round(($data1['SUM(pribl)']*100)/($data2['SUM(pribl)']+$data22['SUM(pribl)']));
                  // $procent = ($data1['SUM(pribl)']*100)/$datapl['plan'];
        ...
        <div class="box">
                ...
                from sales11  WHERE region = '$region' AND adress = '$shop' AND statustovar IS NULL  GROUP BY saler  ");
                $i = 1;
                while($data1 = mysqli_fetch_array($result)){
                  $procent = round(($data1['SUM(pribl)']*100)/($data2['SUM(pribl)']+$data22['SUM(pribl)']));
                  // $procent = ($data1['SUM(pribl)']*100)/$datapl['plan'];
