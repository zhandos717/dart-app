...
?>
    ...
    <div class="content-wrapper">
      ...
      <section class="content-header">
        <h1>
          Магазин <?= $region; ?>-<?= $shop; ?>, ОТЧЕТ за МАРТ 2020
        </h1>
        <ol class="breadcrumb">
                  ...
                  <table id="example2" class="table table-bordered table-hover">
                    <thead>
                      <tr style="background: #398ebd; color: white;">
                        ...
                        <th>Приход.Сумма</th>
                        <th>ПРИБЫЛЬ</th>
                        <th>Колличество проданных<br />товаров</p>
                        </th>
                        <th>АУКЦИОНИСТ +/-</th>
                      </tr>
                    </thead>
                    <tbody>
                      ...
                      $result = mysqli_query($connect, "SELECT COUNT(*), id, region, data, summazaden, SUM(summazaden), SUM(summaprihod),SUM(summakredit),SUM(summareal),SUM(pribl)
                                        from sales03  WHERE region = '$region' AND adress = '$shop' GROUP BY data ");
                      ...
                      ?>
                        ...
                        <tr>
                          ...
                          <td><a href="detail03.php?region=<?= $region; ?>&shop=<?= $shop; ?>&data_z=<?= $data1['data']; ?>"><?= date("d.m.Y", strtotime($data1['data'])); ?></a></td>
                    ...
                    <tfoot>
                      <tr style="background: #398ebd; color: white;">
                        ...
                        $result2 = mysqli_query($connect, " SELECT id, region, SUM(summazaden), SUM(summaprihod),SUM(summakredit),SUM(summareal),SUM(pribl)
                                        from sales03  WHERE region = '$region' AND adress = '$shop' ");
