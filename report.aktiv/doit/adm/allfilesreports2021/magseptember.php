...
if (isset($_SESSION['logged_user'])) :   //если сущесттвует пользователь
  ...
  if ($_SESSION['logged_user']->status == 3) :
    ...
    $result22 = mysqli_query($connect, " SELECT SUM(summareal), SUM(summaprihod), SUM(pribl), SUM(summazaden) from sales09  ");
...
?>
    ...
    <div class="content-wrapper">
      ...
      <section class="content-header">
        <h1>
          Отчет МАГАЗИНОВ за Сентябрь 2020
        </h1>
        <ol class="breadcrumb">
                  ...
                  <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                      ...
                      $result = mysqli_query($connect, "SELECT COUNT(*), region, adress, SUM(summareal), SUM(summaprihod), SUM(pribl)
                                                        from sales09 WHERE fromtovar = 1 GROUP BY adress ");
...
// COUNT(*), id,  data, summazaden, SUM(summazaden), SUM(),,,
                      ...
                      ?>
                        <tr>
                          ...
                          <td><a href="viewshop09.php?region=<?= $data1['region']; ?>&shop=<?= $data1['adress']; ?>&from=1"><?= $data1['region']; ?>-<?= $data1['adress']; ?></a></td>
                    ...
                    <tfoot>
                      <?
                      $result2 = mysqli_query($connect, " SELECT SUM(summareal), SUM(summaprihod), SUM(pribl), SUM(summazaden) from sales09 WHERE fromtovar = 1  ");
                ...
                <div class="layer">
                  <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                      <?
                      $result = mysqli_query($connect, "SELECT COUNT(*), region, adress, SUM(summareal), SUM(summaprihod), SUM(pribl)
                                                        from sales09 WHERE fromtovar = 2  GROUP BY adress ");
...
// COUNT(*), id,  data, summazaden, SUM(summazaden), SUM(),,,
                      ...
                      ?>
                        <tr>
                          <td><i class="fa fa-shopping-cart"></i></td>
                          <td><a href="viewshop09.php?region=<?= $data1['region']; ?>&shop=<?= $data1['adress']; ?>&from=2"><?= $data1['region']; ?>-<?= $data1['adress']; ?></a></td>
                          <td><?= number_format($data1['SUM(summareal)'], 0, '.', ' '); ?></td>
                          <td><?= number_format($data1['SUM(summaprihod)'], 0, '.', ' '); ?></td>
                          <td><?= number_format($data1['SUM(pribl)'], 0, '.', ' '); ?></td>
                          <td><?= $data1['COUNT(*)']; ?></td>
                    ...
                    <tfoot>
                      <?
                      $result2 = mysqli_query($connect, " SELECT SUM(summareal), SUM(summaprihod), SUM(pribl), SUM(summazaden) from sales09 WHERE fromtovar = 2  ");
                      $data2 = mysqli_fetch_array($result2);
                      $aukminus = $data2['SUM(summareal)'] - $data2['SUM(summazaden)'];
                      ?>
                      <tr style="background: #d3d7df; color: black;">
