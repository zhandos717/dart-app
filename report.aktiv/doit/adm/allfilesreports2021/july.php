...
if ($_SESSION['logged_user']->status == 3) :
  ...
  //   return $response;
  // };
  $data_begin = '2021-07-01'; //Дата начало
  $data_end   = '2021-07-31'; //Дата конец
  $day =  '2021-08-04';
                ...
                <table class="table table-bordered table-hover" style="white-space:nowrap;" id="datatable-tabletools">
                  ...
                  <tbody>
                    ...
                    while ($data1 = mysqli_fetch_array($result)) {
                      ...
                      <tr>
                        <td><a href="viewreportregion07.php?region=<?= $region  ?>"> <?= $region  ?></a></td>
                        <td>
                        ...
                        <?
                        $result4 = R::getCol("SELECT  adress FROM reports072021 WHERE region='$region' GROUP BY adress ");
                        ...
                        foreach ($result4 as $filial) {
                          $nal = R::findOne('reports072021', 'adress =? ORDER BY data DESC LIMIT 1', [$filial]);
                          $s += $nal['auktech'];
                          $s2 += $nal['aukshubs'];
                          $s3 += $nal['nalvzaloge'];
                          // $result5 = mysqli_query($connect, " SELECT auktech,aukshubs,nalvzaloge FROM reports072021 WHERE segdata=(SELECT MAX(segdata) FROM reports072021 WHERE region = '$region' AND adress = '$filial' ) ");
                  ...
                  <tfoot>
                    <? $result2 = mysqli_query($connect, " SELECT id, region, SUM(dl),SUM(dm),SUM(dop), SUM(dk), SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv) from reports072021  ");
            ...
            <div class="box-body">
              <div class="table-responsive">
                <table class="table table-bordered table-hover" id="example1">
                  ...
                  <tbody>
                    <?
                    $result = R::getAll("SELECT COUNT(*) as count , SUM(p1), region FROM tickets WHERE NOT (status = '11' OR status = '1' ) AND (region = 'Караганда' OR region = 'Кокшетау' OR region = 'Костанай' OR region = 'Павлодар' OR region = 'Нур-Султан')
                    ...
                    foreach ($result as $data1) {
                      ...
                      $rashod = R::getCol("SELECT SUM(tekrashod) FROM comisstest  WHERE region = '$region' AND data BETWEEN '$data_begin' AND '$data_end' ");
                    
                    ?>
                      <tr>
                        <td> <a class="btn btn-block bg-olive" href="viewreportregion07.php?region=<?= $data1['region']; ?>"><?= $data1['region']; ?></a></td>
                        <td>
                        ...
                        <td>
                          <?= number_format($rashod[0], 0, '.', ' ');
                          $tekrashod += $rashod[0];  ?></td>
                        <td class="info">
                          <?= number_format($ch- $rashod[0], 0, '.', ' ');
                          $total_ch += $ch; ?>
                        </td>
                  ...
                  <tfoot>
                    <tr class="bg-gray">
                      ...
                      <td class="bg-blue"><?= number_format($total_ch- $tekrashod, 0, '.', ' '); ?></td>
                ...
                <table class="table table-bordered table-hover" style="white-space:nowrap;">
                  ...
                  <tbody>
                    ...
                    foreach ($result1 as $data2) {
                      $region = $data2['region'];
                      $sales_tbs = R::getAll("SELECT SUM(pribl),COUNT(*) FROM sales WHERE regionlombard = '$region' AND fromtovar = '2' AND data BETWEEN '$data_begin' AND '$data_end'  AND statustovar IS NULL   ");
                    ...
                    ?>
                      <tr>
                        ...
                        <td class="danger"><?= number_format($sale_tbs['SUM(pribl)'], 0, '.', ' ');
                                            $dohodml_tbs += $sale_tbs['SUM(pribl)']; ?></td>
