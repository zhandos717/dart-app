...
if ($_SESSION['logged_user']->status == 3) :
  include "header.php";
  include "menu.php";
  $data_begin = '2021-06-01'; //Дата начало
  $data_end   = '2021-06-31'; //Дата конец
  $day =  $data_end;
                ...
                <table class="table table-bordered table-hover" style="white-space:nowrap;" id="datatable-tabletools">
                  ...
                  <tbody>
                    ...
                    while ($data1 = mysqli_fetch_array($result)) {
                      ...
                      <tr>
                        <td><a href="viewreportregion05.php?region=<?= $region  ?>"> <?= $region  ?></a></td>
                        <td>
                        ...
                        <?
                        $result4 = R::getCol("SELECT  adress FROM reports062021 WHERE region='$region' GROUP BY adress ");
                        ...
                        foreach ($result4 as $filial) {
                          $nal = R::findOne('reports062021', 'adress =? ORDER BY data DESC LIMIT 1', [$filial]);
                          $s += $nal['auktech'];
                          $s2 += $nal['aukshubs'];
                          $s3 += $nal['nalvzaloge'];
                          // $result5 = mysqli_query($connect, " SELECT auktech,aukshubs,nalvzaloge FROM reports062021 WHERE segdata=(SELECT MAX(segdata) FROM reports062021 WHERE region = '$region' AND adress = '$filial' ) ");
                  ...
                  <tfoot>
                    <? $result2 = mysqli_query($connect, " SELECT id, region, SUM(dl),SUM(dm),SUM(dop), SUM(dk), SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv) from reports062021  ");
              ...
              <div class="table-responsive">
                <table class="table table-bordered table-hover" id="example1">
                  ...
                  <tbody>
                    ...
                    ?>
                      <tr>
                        <td><a class="btn btn-block bg-olive" href="viewreportregion05.php?region=<?= $data1['region']; ?>"><?= $data1['region']; ?></a></td>
                        <td>
