                  ...
                  <div class="row">
                    ...
                    <div class="col-lg-3 col-xs-6">
                      <div class="input-group">
                        ...
                        <select class="form-control" id="region1" name="region">
                    ...
                    <div class="col-lg-3 col-xs-6">
                      <div class="input-group">
                        ...
                        <select class="form-control" id="adress1" name="adress"></select>
                      </div>
                      <!-- /input-group -->
                    </div>
                    <div class="col-lg-3 col-xs-6">
                  ...
                  <div class="row">
                    <div class="col-lg-3 col-xs-6">
                      <div class="input-group">
                        ...
                        <input class="form-control" type="text" required name="summa" placeholder="Сумма опрерации">
                      </div>
                      <!-- /input-group -->
                    </div>
      ...
      <div class="box-body">
        ...
        <div class="table-responsive">
          <table id="example1" class="table table-bordered table-hover">
            ...
            <tbody id="otvet">
              <?
              $result = mysqli_query($connect, "SELECT *FROM finance  ORDER BY id DESC"); //WHERE status = 1 OR status = 2
              $i = 1;
              while ($data = mysqli_fetch_array($result)) {
                $np = $data['np'];
                $operation = $data['operationtype'];
                $result1 = mysqli_query($connect, "SELECT *FROM finance WHERE np ='$np' AND NOT operationtype = '$operation'  ORDER BY id DESC");
            ...
            <tfoot>
              <?
              $result2 = mysqli_query($connect, "SELECT SUM(summa) FROM finance WHERE kassaoperation = '1' AND operationtype = '0' ");
              $data2 = mysqli_fetch_array($result2);

              $result3 = mysqli_query($connect, "SELECT SUM(summa) FROM finance WHERE kassaoperation = '1' AND operationtype = '1' ");
              $data3 = mysqli_fetch_array($result3);
              ?>
              <tr>
                <th colspan="2" class="text-center">ИТОГ:</th>
                <th colspan="2" class="text-center">пополнение:</th>
                <th colspan="2"><?= number_format($data2['SUM(summa)'], 0, '.', ' '); ?></th>
                <th colspan="2" class="text-center">Изъятие:</th>
                <th colspan="2"><?= number_format($data3['SUM(summa)'], 0, '.', ' '); ?></th>
              </tr>
            </tfoot>
          </table>
            ...
            <tbody>
              ...
              foreach ($result as $data) { ?>
                <tr>
                  ...
                  <td><?= $data['filial'] ?></td>
                  <td><?= $data['kassa'] ?></td>
                  <td><?= number_format($data['startamount'], 0, '.', ' ') ?> тенге</td>
                  <td><?= number_format($data['cashbox'], 0, '.', ' ') ?> тенге</td>
                </tr>
              <? }; ?>
            </tbody>
          </table>
