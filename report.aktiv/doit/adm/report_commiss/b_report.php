...
$result5 = mysqli_query($connect, " SELECT SUM(summa_vydachy) FROM tickets  WHERE (status = '7' OR status = '10' OR status = '14' OR status = '15' ) AND NOT type = 'Шубы'  ");
...
$result12 =$mysqli->query("SELECT SUM(summa_vydachy) FROM tickets WHERE status = 2 ");
$data12 = mysqli_fetch_array($result12);

$ss3 = $data12['SUM(summa_vydachy)'];


$result12 =$mysqli->query("SELECT SUM(p1),SUM(proc),SUM(summa_vydachy),SUM(cena_pr),COUNT(*) as count FROM tickets WHERE NOT  status = 11 AND NOT  status = 1 ");
$data2 = mysqli_fetch_array($result12);

$result12 =$mysqli->query("SELECT SUM(p1),SUM(proc),SUM(summa_vydachy),SUM(cena_pr) FROM tickets WHERE  status = 5 ");
$data3 = mysqli_fetch_array($result12);
$chistaya = ($data2['SUM(p1)']+$data2['SUM(proc)'])+($data3['SUM(cena_pr)']-$data3['SUM(summa_vydachy)']); // чистая прибыль
?>
           ...
           <div class="box-body">
             <div class="table-responsive" >
               <table class="table table-bordered table-hover" style="white-space:nowrap;">
                 ...
                 <tbody>
                   <?
                    $result = mysqli_query($connect, " SELECT SUM(p1),SUM(proc), region FROM tickets WHERE NOT region IS NULL  GROUP BY region  ");

                    while ($data1 = mysqli_fetch_array($result)) {
                      $region = $data1['region'];

                      $result12 =$mysqli->query("SELECT COUNT(*) as count ,SUM(summa_vydachy)  FROM tickets WHERE region = '$region' AND NOT  status = 11 ");
                      ...
                      $result13 =$mysqli->query("SELECT SUM(summa_vydachy) FROM tickets WHERE region = '$region' AND  status = 4 ");
                      ...
                      $result5 = mysqli_query($connect, " SELECT SUM(summa_vydachy) FROM tickets  WHERE region = '$region'  AND status = '2'  ");
                      $data5 = mysqli_fetch_array($result5);


                      $result8 = mysqli_query($connect, " SELECT SUM(summa_vydachy), SUM(cena_pr) FROM tickets  WHERE region = '$region'  AND status = '5' ");
                      $data8 = mysqli_fetch_array($result8);

                    ?>
                     <tr>
                       <td><a type="button" href="a_report_commiss.php?id=3&region=<?=$data1['region'];?>" ><?=$data1['region'];?></a></td>
                         <td>
                      ...
                      <td>
                       ...
                       <?
                        $chistaya = $data1['SUM(p1)']+$data1['SUM(proc)']+($data8['SUM(cena_pr)']-$data8['SUM(summa_vydachy)']);                                                  // чистая прибыль  = за минусом 20 процентов
                        ?>
