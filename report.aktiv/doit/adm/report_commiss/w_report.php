              ...
              <div class="box-body">
                <div class="">
                  ...
                  <table class="table table-hover table-bordered" id="datatable-tabletools">
                    ...
                    <tbody>
                      <? $data = R::findLike('tickets',['status' =>[7,10,14,15], 'region'=>['Нур-Султан','Костанай','Караганда','Павлодар','Кокшетау'] ],'ORDER BY dv ASC');
                         $i = 1;
                         foreach($data as $param){
                         $day = (int) round((strtotime(date('Y-m-d')) - strtotime($param['dv'])) / (60 * 60 * 24));
                          
                        if($day < 1 ){$color1 = 'blue'; $count += 1;}
                        elseif($day > 2 AND  $day < 4){$color1 = 'blue'; $count11 += 1;}
                        elseif($day >= 6 AND $day < 8){ $color = 'success'; $color1 = 'olive'; $count1 += 1; }
                        elseif($day >= 8 && $day < 10){$color = 'warning'; $color1 = 'yellow'; $count2 += 1;}
                        elseif($day >= 10){$color = 'danger'; $color1 = 'red'; $count3 += 1;}
                      ?>
                      <tr class="<?= $color; ?>">
                        <td> <?= $i++; ?>.</td>
                        <td> <?= $param['nomerzb']; ?></td>
