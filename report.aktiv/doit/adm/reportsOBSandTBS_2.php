...
if ($_SESSION['logged_user']->status == 3) :
  ...
  //     CURLOPT_URL => 'https://report.commission2.kz/doit/api/get_total_report.php',
  ...
  $data_end   = '2022-02-28'; //Дата конец
    ...
    // ]);
        ...
        $day =  date('Y-m-d'); // день
    $month =  date('m', strtotime($data_end));


  // foreach ($report_total as $arr) {

    // $test = R::findOne('comision2region', 'region=? AND data =? AND month =?', [$arr['region'], $day, $month]);
    
    // if($test->summa_vydachy != $arr['summa_vydachy']){
            ...
            <div class="box-body">
              <div class="">
                <table class="table table-bordered table-hover" style="white-space:nowrap;" id="datatable-tabletools">
                  ...
                  <tfoot>
                    <? $result2 = mysqli_query($connect, " SELECT id, region, SUM(dl),SUM(dm),SUM(dop), SUM(dk), SUM(dohod),SUM(allclients),SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),SUM(chv) from reports  ");
