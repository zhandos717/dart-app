...
$result5 = mysqli_query($connect, " SELECT SUM(summa_vydachy) FROM tickets  WHERE region = '$region' AND (status = '7' OR status = '10' OR status = '14' OR status = '15' ) AND NOT type = 'Шубы'  ");
...
$result12 =$mysqli->query("SELECT SUM(summa_vydachy) FROM tickets WHERE region = '$region' AND status = 2 ");
           ...
           <div class="box-body">
             <div class="table-responsive" >
               <table class="table table-bordered table-hover" style="white-space:nowrap;">
                 ...
                 <tbody>
                   <?
                    $result = mysqli_query($connect, " SELECT SUM(p1),SUM(proc), region,adressfil FROM tickets WHERE region = '$region' AND NOT region IS NULL  GROUP BY adressfil  ");

                    while ($data1 = mysqli_fetch_array($result)) {
                      $adress = $data1['adressfil'];

                      $result12 =$mysqli->query("SELECT COUNT(*) as count ,SUM(summa_vydachy)  FROM tickets WHERE adressfil = '$adress' AND NOT  status = 11 ");
                    ...
                    ?>
                     <tr>
                       <td><a type="button" href="a_report_commiss.php?id=4&adress=<?=$data1['adressfil'];?>" class="btn  btn-block bg-olive btn-flat"><b><?=$data1['adressfil'];?></b></a></td>
                         <td>
