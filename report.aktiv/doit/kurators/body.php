                                ...
                                <tbody>
                                    ...
                                    foreach ($result as $data1) {
                                        ...
                                        if (!empty($comis2)) {
                                            ...
                                            $result12 = $mysqli->query("SELECT SUM(summa_vydachy),SUM(p1),SUM(proc)  FROM tickets WHERE adressfil = '$adress' AND NOT (status = '11' OR status = '1') AND dataseg BETWEEN '$data_begin' AND '$data_end' ");
                                            $data89 = mysqli_fetch_array($result12);
                                            $result19 = $mysqli->query("SELECT SUM(proc)  FROM tickets WHERE adressfil = '$adress' AND status = '4' AND datavykup BETWEEN '$data_begin' AND '$data_end' ");
                                            $data19 = mysqli_fetch_array($result19);
                                            $result81 = mysqli_query($connect, " SELECT SUM(summa_vydachy), SUM(cena_pr),SUM(profit) FROM tickets  WHERE adressfil = '$adress' AND status = '5' AND datesale BETWEEN '$data_begin' AND '$data_end'  ");
                                        ...
                                        $chistaya_tbs += $chistaya;
                                    ?>
                                        <tr>
                                            <td><a class="btn btn-block bg-olive" href="viewfilial.php?region=<?= $region; ?>&adress=<?= $data1['adress']; ?>"><?= $data1['adress']; ?></a></td>
                                            <td>
                                            ...
                                            <th class="danger"><?= number_format($chistaya, 0, '.', ' ');?></th>
                                            <th class="success"><?= number_format($chistaya_lombard + $chistaya, 0, '.', ' '); ?></th>
