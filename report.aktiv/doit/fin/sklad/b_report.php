...
<? //проверка существовании сессии
  ...
  if($_POST['date1'] AND $_POST['date2']){
    ...
    $result12 = mysqli_query($connect,"SELECT region,adress, MIN(summstart), SUM(finhelp),SUM(comis),SUM(proc),SUM(vydacha),SUM(vozvrat),SUM(summsale),SUM(withdrawal),SUM(salesincome),SUM(summstart),SUM(endsumm) FROM repotscom WHERE datereport BETWEEN '$data1' AND '$data2'  GROUP BY region ");
          ...
          <div class="col-md-12">
             <div class="box box-primary">
               <div class="box-header with-border">
                 ...
                 <div class="box-body">
                   <form action="a_report.php?id=1" method="POST">
                    ...
                    <thead>
                        ...
                        <tr>
                          ...
                          <tbody style="white-space:nowrap;">
                            ...
                            <a type="button" href="a_report.php?id=3&region=<?=$data12['region'];?>" class="btn  btn-block bg-olive btn-flat"><b><?=$data12['region'];?></b></a>
                          ...
                          $result12 = mysqli_query($connect,"SELECT adress, SUM(summstart), SUM(finhelp),SUM(comis),SUM(proc),SUM(vydacha),SUM(vozvrat),SUM(summsale),SUM(withdrawal),SUM(salesincome),SUM(summstart),SUM(endsumm) FROM repotscom WHERE datereport BETWEEN '$data1' AND '$data2'  ");
                          $data12 = mysqli_fetch_array($result12);

                          $result16 = mysqli_query($connect,"SELECT SUM(summstart) FROM repotscom WHERE datereport ='$data1' ");
                          $data16 = mysqli_fetch_array($result16);

                          $result15 = mysqli_query($connect,"SELECT adress, SUM(summstart), SUM(finhelp),SUM(comis),SUM(proc),SUM(vydacha),SUM(vozvrat),SUM(summsale),SUM(withdrawal),SUM(salesincome),SUM(summstart),SUM(endsumm) FROM repotscom WHERE datereport ='$data2' ");
