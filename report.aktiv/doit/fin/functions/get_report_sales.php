...
if (!empty($_POST['regions']) && empty($_POST['adresses'])) {


    $sql = 'statustovar IS NULL 
    AND region IN( "'.implode('","', $_POST['regions']).'" ) 
    AND data BETWEEN :date1 
    AND :date2 ORDER BY id DESC';
    $array = [':date1' => $_POST['date1'], ':date2' => $_POST['date2']];

} else if (!empty($_POST['regions']) && !empty($_POST['adresses'])) {
   
    $sql = 'statustovar IS NULL 
    AND  adress IN( "'.implode('","', $_POST['adresses']).'" )  
    AND data BETWEEN :date1 AND :date2 ORDER BY id DESC';
