<? //проверка существовании сессии
  ...
  if($_POST['date1'] AND $_POST['date2']){
                                    ...
                                    if($region == 'Все' AND $status != 'Все')  {
                                    $result = mysqli_query($connect,"SELECT * FROM tickets WHERE status = '$status' AND  dataseg BETWEEN '$data1' AND '$data2'  ");
                                    $comment = $region.' / '.$adress.' за период с '. date("d.m.Y", strtotime($data1)). ' по ' .date("d.m.Y", strtotime($data2));

                                    $result12 = mysqli_query($connect,"SELECT  SUM(summa_vydachy),SUM(cena_pr) FROM tickets WHERE status = '$status' AND  dataseg BETWEEN '$data1' AND '$data2'  ");
                                    $data22 = mysqli_fetch_array($result12);
                                    }elseif ($region == 'Все' AND $status == 'Все' ) {
                                    $result = mysqli_query($connect,"SELECT * FROM tickets WHERE  dataseg BETWEEN '$data1' AND '$data2' ");
                                    $comment = $region.' / '.$adress.' за период с '. date("d.m.Y", strtotime($data1)). ' по ' .date("d.m.Y", strtotime($data2));

                                    $result12 = mysqli_query($connect,"SELECT  SUM(summa_vydachy),SUM(cena_pr) FROM tickets WHERE  dataseg BETWEEN '$data1' AND '$data2' ");
                                    $data22 = mysqli_fetch_array($result12);
                                    }elseif($region != 'Все' AND $status != 'Все' AND $adress == 'Все'){
                                      ...
                                      $result = mysqli_query($connect,"SELECT * FROM tickets WHERE region = '$region' AND adressfil = '$adress' AND status = '$status' AND  dataseg BETWEEN '$data1' AND '$data2'   ");
                                      $comment = $region.' / '.$adress.' за период с '. date("d.m.Y", strtotime($data1)). ' по ' .date("d.m.Y", strtotime($data2));

                                      $result12 = mysqli_query($connect,"SELECT  SUM(summa_vydachy),SUM(cena_pr) FROM tickets  WHERE region = '$region' AND adressfil = '$adress' AND status = '$status' AND  dataseg BETWEEN '$data1' AND '$data2'   ");
                                      $data22 = mysqli_fetch_array($result12);
                                    };
                                  ?>
      ...
      <div class="col-md-12">
        <div class="box box-primary">
          <div class="box-header">
            <a href="commis/print_report.php" target="_blank" title="Выгрузка в эксель" class="btn btn-success"><i class="fa fa-file-excel-o"></i></a>
