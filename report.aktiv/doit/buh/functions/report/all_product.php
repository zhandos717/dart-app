...
<div class="table-responsive">
    <table id="datatable-tabletools" class="table table-hover table-bordered">
        ...
        <tbody>
            ...
            ?>
            <tr <?if($data_zb['srok']==0):?> class="info"
                <?endif?>>
                ...
                <td><?= number_format($data_zb['summa_vydachy'], 0, '.', ' ');
                    $summa_vydachy += $data_zb['summa_vydachy']; ?></td>
                <td><?= number_format($data_zb['cena_pr'], 0, '.', ' ');
                    $cena_pr += $data_zb['cena_pr']; ?></td>
                <td><?= date("d.m.Y", strtotime($data_zb['reg_data'])); ?></td>
                <td><?= date("d.m.Y", strtotime($data_zb['dv'])); ?></td>
                <td>
                    <? if($data_zb['dateshop']){echo date("d.m.Y", strtotime($data_zb['dateshop']));}else { echo '--'; } ?>
                </td>
                <td><?= $data_zb['eo']; ?></td>
                <td><?= $statuszb['name']; ?></td>
            </tr>
        ...
        <tfoot>
            <tr class="danger">
                ...
                <th colspan="3" class="text-center">Сумма выдачи: <?= number_format($summa_vydachy, 0, '.', ' '); ?> тг.</th>
                <th colspan="3" class="text-center">Сумма продажи: <?= number_format($cena_pr, 0, '.', ' '); ?> тг.</th>
                <th colspan="2" class="text-center">Доход: <?= number_format($summa_vydachy - $cena_pr, 0, '.', ' '); ?> тг.</th>
            </tr>
        </tfoot>
    </table>
