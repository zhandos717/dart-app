...
$result = R::getAll(
  "SELECT adress, MIN(summstart), SUM(finhelp),SUM(comis),
  SUM(proc),SUM(vydacha),SUM(vozvrat),SUM(summsale),
  SUM(withdrawal),SUM(salesincome),SUM(summstart),SUM(endsumm) 
FROM repotscom 
WHERE region = :region
AND datereport BETWEEN  :data1 AND :data2 GROUP BY adress",
  [
    ':region' => $_GET['region'],
    ':data1' => $data1,
    ':data2' => $data2
        ...
        <div class="box box-primary">
          ...
          <div class="box-body">
            <form action="a_report.php" method="GEt">
              ...
              <input type="text" hidden name="region" value="<?= $_GET['region']; ?>">
                ...
                <tbody style="white-space:nowrap;">
                  ...
                  foreach ($result as $data) {
                    ...
                    $summstart_region = R::getCell(
                      "SELECT SUM(summstart) FROM repotscom 
                    WHERE adress= :adress  
                    AND region= :region  
                    ...
                    $endsumm_region = R::getCell("SELECT SUM(endsumm) FROM repotscom WHERE adress='$adress'  AND datereport = '$data2' ");
                    $p1_ticket = R::getCell("SELECT SUM(p1)  FROM tickets WHERE NOT status IN (1,11) AND adressfil = '$adress' AND  dataseg BETWEEN '$data1' AND  '$data2' ");
                    $proc_ticket = R::getCell("SELECT SUM(proc)  FROM tickets WHERE  status = 4 AND adressfil = '$adress' AND  datavykup BETWEEN '$data1' AND  '$data2' ");

                    // var_dump($proc_ticket);
                  ?>
                    <tr>
                      ...
                      <td>
                        ...
                        <a type="button" href="a_report.php?id=4&adress=<?= $data['adress']; ?>&date1=<?= $data1 ?>&date2=<?= $data2 ?>" class="btn  btn-block bg-olive btn-flat"><b><?= $data['adress']; ?></b></a>
                      </td>
