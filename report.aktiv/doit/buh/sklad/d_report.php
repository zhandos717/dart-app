...
$result = R::getAll(
  "SELECT kassa,adress, MIN(summstart), SUM(finhelp),SUM(comis),
SUM(proc),SUM(vydacha),SUM(vozvrat),SUM(summsale),
SUM(withdrawal),SUM(salesincome),SUM(summstart),SUM(endsumm) 
FROM repotscom 
WHERE adress = :adress
AND datereport BETWEEN  :data1 AND :data2 GROUP BY kassa",
  [
    ':adress' => $_GET['adress'],
    ':data1' => $data1,
    ':data2' => $data2
              ...
              <table id="datatable-tabletools" class="tableas table table-hover table-bordered text-center">
                ...
                <tbody style="white-space:nowrap;">
                  ...
                  foreach ($result  as $data) {
                    ...
                    $summstart_region = R::getCell(
                      "SELECT summstart FROM repotscom 
                    WHERE adress= :adress  
                    AND kassa= :kassa  
                    AND datereport = :data1 ",
                      [
                        ':adress' => $adress,
                        ':kassa' => $kassa,
                        ':data1' => $data1,
                    ...
                    $endsumm_region = R::getCell("SELECT endsumm 
                    FROM repotscom 
                    WHERE adress= :adress  
                    AND kassa= :kassa  
                    AND datereport = :data2 ", [
                      ':adress' => $adress,
                      ':kassa' => $kassa,
                      ':data2' => $data2,
                    ]);
                    ...
                    $p1_ticket = R::getCell(
                      "SELECT SUM(p1)  FROM tickets 
                    WHERE NOT status IN (1,11) 
                    AND adressfil = :adress
                    AND kassa= :kassa   
                    AND  dataseg BETWEEN :data1 AND  :data2 ",
                      [
                        ':adress' => $adress,
                        ':kassa' => $kassa,
                        ':data1' => $data1,
                    ...
                    $proc_ticket = R::getCell("SELECT SUM(proc)  FROM tickets 
                    WHERE  status = 4 
                    AND adressfil = :adress
                    AND kassa= :kassa  
                    AND  datavykup BETWEEN :data1 AND  :data2 ", [
                      ':adress' => $adress,
                      ':kassa' => $kassa,
                      ':data1' => $data1,
                      ':data2' => $data2,
                  ...
                  ?>
                    <tr>
                      <td><?= $i++; ?>.
                        <button class="btn btn-primary fix" value="<?= $data['kassa'] ?>"> <i class="fa fa-refresh" style="font-size:24px"></i> </button>
                      </td>
                      <td>
                        <a type="button" class="btn  btn-block bg-olive btn-flat"><b><?= $data['adress']; ?></b></a>
                      </td>
                      <td>
                        <a type="button" class="btn  btn-block bg-olive btn-flat"><b><?= $data['kassa']; ?></b></a>
                      </td>
                      ...
                      <td>
                        <? if ($data['SUM(proc)'] != $proc_ticket && $proc_ticket > 0) {
        ...
        <div class="box box-primary">
          ...
          <div class="box-body">
            <div class="table-responsive">
              <table id="" class="tableas table table-hover table-bordered text-center">
                <thead>
                  <tr>
                    <th rowspan="2" class="success">№</th>
                    <th rowspan="2" class="success">Филиал</th>
                ...
                <tbody style="white-space:nowrap;">
                  ...
                  $i = 1;
                  $result = R::getAll(
                    "SELECT datereport,kassa,adress,summstart,finhelp,comis,
                      proc,vydacha,vozvrat,summsale,
                      withdrawal,salesincome,summstart,endsumm
                      FROM repotscom 
                      WHERE adress = :adress
                  ...
                  foreach ($result  as $data) {
                    ...
                    $summ = R::getRow(
                      "SELECT summstart , endsumm FROM repotscom 
                    WHERE adress= :adress  
                    AND kassa= :kassa  
                    AND datereport = :data1 ",
                      [
                    ...
                    $proc_ticket = R::getCell(
                      "SELECT SUM(proc) FROM tickets 
                    WHERE status = 4 
                  ...
                  ?>
                    <tr>
                      <td><?= $i++; ?>.</td>
                      <td>
                        <a type="button" class="btn  btn-block bg-olive btn-flat"><b><?= $data['adress'] . '/' . $data['kassa']; ?></b></a>
                      </td>
                      <td>
                        <a type="button" class="btn  btn-block bg-olive btn-flat"><b><?= $data['datereport']; ?></b></a>
                      </td>
                      <td><?= number_format($summ['summstart'], 0, '.', ' '); ?></td>
                      <td><?= number_format($data['vydacha'], 0, '.', ' ');
                          $vydacha1 += $data['vydacha']; ?></td>
                      <td><?= number_format($data['vozvrat'], 0, '.', ' ');
                          $vozvrat1 += $data['vozvrat']; ?></td>
                      <td><?= number_format($data['finhelp'], 0, '.', ' ');
                          $finhelp1 += $data['finhelp']; ?></td>
                      <td><?= number_format($data['withdrawal'], 0, '.', ' ');
                          $withdrawal1 += $data['withdrawal'];  ?></td>
                      <td><?= number_format($summ['endsumm'], 0, '.', ' '); ?></td>
                      <td>
                        <? if ($data['comis'] == $p1_ticket) {
                          echo number_format($data['comis'], 0, '.', ' ');
                        } else {

                          echo 'Сумма в отчете: ' . number_format($data['comis'], 0, '.', ' ');
                          echo '<br>';
                          echo 'Сумма в сводке: ' . number_format($p1_ticket, 0, '.', ' ');


                          echo '<br> Есть расхождение на сумму: ' . ($data['comis'] - $p1_ticket);
                        };
                        $comis1 += $data['comis']; ?>
                      </td>
                      <td>

                        <? if ($data['proc'] != $proc_ticket && $proc_ticket > 0) {
                          echo 'Сумма в отчете: ' . number_format($data['proc'], 0, '.', ' ');
                          echo '<br>';
                          echo 'Сумма в сводке: ' . number_format($proc_ticket, 0, '.', ' ');
                          echo '<br> Есть расхождение на сумму: ' . ($data['proc'] - $proc_ticket);
                        } else {
                          echo number_format($data['proc'], 0, '.', ' ');
                        };

                        $proc1 += $data['proc']; ?>
                      </td>
                      <td><?= number_format($data['summsale'], 0, '.', ' ');
                          $summsale1 += $data['summsale']; ?></td>
                      <td><?= number_format($data['salesincome'], 0, '.', ' ');
                          $salesincome1 += $data['salesincome']; ?></td>
                    </tr>
                  <? } ?>
                </tbody>
                <tfoot>
                  <tr style="white-space:nowrap;">
                    <th colspan="3"> Итого:</th>
                    <th><?= number_format($summ['summstart'], 0, '.', ' '); ?></th>
                    <th><?= number_format($vydacha1, 0, '.', ' '); ?></th>
                    <th><?= number_format($vozvrat1, 0, '.', ' '); ?></th>
                    <th><?= number_format($finhelp1, 0, '.', ' '); ?></th>
                    <th><?= number_format($withdrawal1, 0, '.', ' '); ?></th>
                    <th><?= number_format($summ['endsumm'], 0, '.', ' '); ?></th>
                    <!-- <th>0</th> -->
                    <th><?= number_format($comis1, 0, '.', ' '); ?></th>
                    <th><?= number_format($proc1, 0, '.', ' '); ?></th>
                    <th><?= number_format($summsale1, 0, '.', ' '); ?></th>
                    <th><?= number_format($salesincome1, 0, '.', ' '); ?></th>
                    <!-- <th>0</th> -->
                  </tr>
                </tfoot>
              </table>
...
<script>
  ...
  $(function() {
    $('.fix').click(function() {
      ...
      $.post('functions/fix_total_report.php', {
          kassa: kassa,
          adress: $_get('adress'),
          date1: $_get('date1'),
          date2: $_get('date2'),
