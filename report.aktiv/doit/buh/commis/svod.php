...
<script type="text/javascript">
    ...
    syncList3.dataList = {
        <? $city = R::find('kassa', 'GROUP BY region');
        foreach ($city as $region) {?> 
        '<?= $region["region"] ?>': {
        <? $filial = R::find('kassa', 'region=? AND statuskassa <> 9  GROUP BY filial', [$region["region"]]);
          if(!empty($adress)){?> '<?=$adress?>' : '<?=$adress?>', <?}
        foreach($filial as $fil) {?>'<?= $fil["filial"] ?>' : '<?= $fil["filial"] ?>', 
        <?}?>},
        <?}
        $filial = R::find('kassa', 'GROUP BY filial');
        foreach ($filial as $fil) {
        ?> '<?= $fil["filial"] ?>': {
        <? $filial = R::find('kassa', 'filial=? GROUP BY kassa', [$fil["filial"]]);
        if(!empty($kassa)){?> '<?=$kassa?>' : '<?=$kassa?>', <?}
        foreach($filial as $fil) {?> 
        '<?= $fil["kassa"] ?>' : '<?= $fil["kassa"] ?>', <?}?> },<?}?>
  };
        syncList3.sync("List3", "List4", "List5");
</script>