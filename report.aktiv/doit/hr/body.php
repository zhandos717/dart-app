...
function percent($number) {
      ...
      $result2 = mysqli_query($connect, " SELECT id, region, SUM(dl),SUM(dm),SUM(dop), SUM(dohod),SUM(stabrashod),SUM(tekrashod),SUM(allclients),
                                          SUM(newclients),SUM(vzs),SUM(vozvrat),SUM(nakladnoy),COUNT(adress),SUM(chv) from reports WHERE `region`='$region'  ");
                ...
                <tbody>
                  <?
                   ...
                   while ($data1 = mysqli_fetch_array($result)) {
                    ...
                    $ras =$table[0];
                    $res = R::getRow("SELECT * FROM reports WHERE adress = '$adress' ORDER BY id DESC LIMIT 1");
                    ...
                    $result12 =$mysqli->query("SELECT SUM(summa_vydachy),SUM(p1),SUM(proc)  FROM tickets WHERE adressfil = '$adress1' AND NOT (status = '11' OR status = '1' ) AND dataseg BETWEEN '$start' AND '$end' ");
                    $data89 = mysqli_fetch_array($result12);
                    $result19 =$mysqli->query("SELECT SUM(proc)  FROM tickets WHERE adressfil = '$adress1' AND status = '4' AND datavykup BETWEEN '$start' AND '$end' ");
                    $data19 = mysqli_fetch_array($result19);
                    $result81 = mysqli_query($connect, " SELECT SUM(summa_vydachy), SUM(cena_pr),SUM(profit) FROM tickets  WHERE adressfil = '$adress1' AND status = '5' AND datesale BETWEEN '$start' AND '$end'  ");
          ...
          <div class="box-body">
            <div class="table-responsive">
              <table class="table table-bordered table-hover" style="white-space:nowrap;">
                ...
                <tbody>
                  <?
                   $result = mysqli_query($connect, " SELECT SUM(p1), region,adressfil FROM tickets WHERE NOT (status = '11' OR status = '1' ) AND dataseg BETWEEN '$start' AND '$end' AND region = '$region1'  GROUP BY adressfil  ");
                  while ($data1 = mysqli_fetch_array($result)) {
                     $adress = $data1['adressfil'];
                     $region = $data1['region'];
                     $result19 = mysqli_query($connect, " SELECT SUM(proc) FROM tickets WHERE status = '4' AND adressfil = '$adress' AND datavykup BETWEEN '$start' AND '$end'  ");
                     $data19 = mysqli_fetch_array($result19);
                     $result12 =$mysqli->query("SELECT COUNT(*) as count ,SUM(summa_vydachy)  FROM tickets WHERE adressfil = '$adress' AND NOT (status = '11' OR status = '1' ) AND dataseg BETWEEN '$start' AND '$end' ");
                     $data89 = mysqli_fetch_array($result12);
                     ...
                     $result13 =$mysqli->query("SELECT SUM(summa_vydachy) FROM tickets WHERE adressfil = '$adress' AND  status = '4' AND dataseg BETWEEN '$start' AND '$end' ");
                     ...
                     $result5 = mysqli_query($connect, " SELECT SUM(summa_vydachy) FROM tickets  WHERE adressfil = '$adress'  AND status = '2' AND dataseg BETWEEN '$start' AND '$end'  ");
                     $data5 = mysqli_fetch_array($result5);
                     $result8 = mysqli_query($connect, " SELECT SUM(summa_vydachy), SUM(cena_pr),SUM(profit) FROM tickets  WHERE adressfil = '$adress' AND status = '5' AND datesale BETWEEN '$start' AND '$end'  ");
