...
}elseif ($_POST['adress'] != 'Все' AND $_POST['region']) {
    if($_POST['status']=='Все'){
...
}else if($_POST['product'] == 2){
    $sql  = 'region =  :region  AND status = 2 AND adressfil =  :adress ';
    $placeholder = [':region' =>$region, ':adress' => $adress];
} else if ($_POST['product'] == 14) {
    $sql  = 'region =  :region  AND status IN(7,10,14,15) AND adressfil =  :adress ';
    $placeholder = [':region' => $region, ':adress' => $adress];
}
