...
if ($_SESSION['logged_user']->status == 1) :
  ...
  if (in_array($region, $reg)) {
    $data89 = R::getRow("SELECT COUNT(*) as count , SUM(summa_vydachy)  FROM tickets WHERE adressfil = '$adress' AND NOT status IN (1,11)  AND (dataseg BETWEEN '$start' AND '$end') ");

    $data8 = R::getRow("SELECT SUM(summa_vydachy), SUM(cena_pr) FROM tickets  WHERE adressfil = '$adress' AND status = 5 AND datesale BETWEEN '$start' AND '$end'  ");

    $nal =  R::getCell("SELECT SUM(summa_vydachy) FROM tickets  WHERE adressfil = '$adress'  AND status = 2 ");
    $shuby = R::getCell("SELECT SUM(summa_vydachy) FROM tickets WHERE adressfil = '$adress' AND status IN (7,10,14,15) AND type = 'Шубы'  ");
    ...
    $proc = R::getCell(" SELECT SUM(proc) FROM tickets WHERE status = '4'  AND datavykup BETWEEN '$start' AND '$end' AND adressfil = '$adress'  ");
    $comis = R::getCell(" SELECT SUM(p1) FROM tickets WHERE NOT  status IN (1,11)  AND dataseg BETWEEN '$start' AND '$end' AND adressfil = '$adress'  ");
    $count = $data89['count'];
    $summa_vydachy = $data89['summa_vydachy'];
    $sale = $data8["SUM(summa_vydachy)"];
    $cena_pr = $data8["SUM(cena_pr)"];
...
?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        <span id="region"><?= $region; ?></span>/<span id="adress"><?= $adress; ?></span> с <?= $start ?> по <?= $end ?>
  ...
  <script>
    ...
    $(function() {
      $.getJSON('/doit/function/get_branches_sales.php').done(function(data) {
