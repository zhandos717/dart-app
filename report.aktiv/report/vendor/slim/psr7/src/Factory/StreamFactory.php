...
class StreamFactory implements StreamFactoryInterface
{
    ...
    public function createStreamFromFile(
        ...
        if (!is_resource($resource)) {
            throw new RuntimeException(
                "StreamFactory::createStreamFromFile() could not create resources from file `$filename`"
            );
        }
    ...
    public function createStreamFromResource($resource, StreamInterface $cache = null): StreamInterface
    {
        if (!is_resource($resource)) {
            throw new InvalidArgumentException(
                'Parameter 1 of StreamFactory::createStreamFromResource() must be a resources.'
            );
        }
