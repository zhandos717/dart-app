...
```sh
composer require nikic/fast-routes
```
...
// This routes is NOT valid, because optional parts can only occur at the end
$r->addRoute('GET', '/user[/{id:\d+}]/{name}', 'handler');
```
...
```php
$r->get('/get-routes', 'get_handler');
$r->post('/post-routes', 'post_handler');
```

Is equivalent to:

```php
$r->addRoute('GET', '/get-routes', 'get_handler');
$r->addRoute('POST', '/post-routes', 'post_handler');
```
...
$dispatcher = FastRoute\cachedDispatcher(function(FastRoute\RouteCollector $r) {
    ...
    'cacheFile' => __DIR__ . '/routes.cache', /* required */
